...
**Arquivo:** `src/components/Apresentacoes/Dashboard/` (ApresentacoesDashboard.tsx, CardApresentacao.tsx, MiniaturaSlideApresentacao.tsx, ModalNovaApresentacao.tsx)
**Tipo:** Client Components
**Uso:** `src/app/PainelAlpha/Apresentacoes/page.tsx` fica fino, só renderiza `<ApresentacoesDashboard temaName={...} />`.
**Miniatura:** `MiniaturaSlideApresentacao` recebe o DTO validado do Slide 1 e monta o mesmo `RenderComponente` da apresentacao dentro de um SVG responsivo. A previa preserva os componentes reais, inclusive background, midias e 3D, normaliza `z-index` negativo e remove apenas animacoes de entrada para que o conteudo apareca imediatamente. O visualizador so e montado quando o card esta proximo da viewport (`useInView`); slide vazio preserva o fallback com `MonitorPlay`.
...
- **Resize de texto escala tipografia**: o gesto usa o snapshot inicial para escalar `fontSize` e os tamanhos explícitos dos runs ricos sem acumular arredondamento. A barra e as laterais usam larguras responsivas; `CanvasArea` recalcula o enquadramento em todo resize real da área.
- **Fontes adicionadas são globais**: `POST /api/apresentacoes/fontes` grava no prefixo compartilhado do Vercel Blob; `fontes-globais.ts` reconstrói o catálogo sem tabela. Editor recebe o catálogo completo, enquanto player/link/exportação filtram somente as famílias usadas.
