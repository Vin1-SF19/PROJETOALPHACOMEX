...
**Pendência real, não resolvida nesta sessão:** as outras 8 relações (`Checklist`, `observacoes_contratos`, `CommissionEntry`/`CommissionDivergence`/`CommissionEvent.businessProcessId`, e toda a árvore `BpmCard*`) continuam com a MESMA constraint física órfã. Hoje não estão gerando erro visível porque nenhum registro novo foi criado nessas tabelas-pai desde o respectivo rename — mas vão falhar da mesma forma assim que o primeiro evento/card/checklist novo for criado para uma empresa recente. Usuário optou por corrigir só Extratos nesta sessão e decidir depois sobre o resto. `PessoaEmpresaVinculo` está fora de escopo (0 linhas, model já removido do schema, tabela física morta).
