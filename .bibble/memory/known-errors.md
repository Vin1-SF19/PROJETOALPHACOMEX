...
## React error #310 ("more hooks than previous render") em produção — `<Suspense>` desnecessário numa página `force-dynamic`
...
**Causa:** um Server Component com `export const dynamic = "force-dynamic"` (nunca prerenderizado estaticamente) envolvia um Client Component grande (dezenas de hooks) dentro de `<Suspense fallback={null}>`. O único motivo real para o Next.js App Router exigir `<Suspense>` ao redor de `useSearchParams()` é permitir prerendering estático parcial — mas como a página já era 100% dinâmica, esse boundary não protegia nada. Ele só causava, na hidratação em produção, uma transição abrupta de contagem de hooks entre o fallback (`null` = 0 hooks) e o conteúdo real (dezenas de hooks) — gatilho clássico do erro #310.
**Fix:** remover o `<Suspense>` (e o import de `Suspense`) por completo — renderizar o Client Component direto no `return` do Server Component. Antes de aplicar, confirmar que a página é `force-dynamic` (se NÃO for e usar `useSearchParams()`, o Suspense é necessário de verdade para o build estático não falhar — não remover nesse caso).
**Contexto:** `src/app/PainelAlpha/Roadmap/page.tsx` (envolvia `RoadmapDashboard.tsx`, que usa `useSearchParams()` para ler `?novoModulo=1`). Regra geral reconhecível: qualquer página futura combinando `export const dynamic = "force-dynamic"` com `<Suspense>` ao redor de um client component é candidata a ter o Suspense removido, a menos que exista necessidade REAL de streaming/loading incremental (nesse caso o fallback deve ser um esqueleto de verdade, nunca `null`, para reduzir a chance do mesmo mismatch).
**Como foi confirmado:** reprodução ao vivo em produção (usuário logou no navegador da sessão do Bibble; Bibble só observou console/network, nunca tocou em credenciais) — `read_console_messages` capturou o erro real, `read_network_requests` confirmou que o SSR retornava 200 (dado chegava bem do servidor), e testar navegação client-side vs. hard-reload isolou a causa para hidratação, não para os dados em si.
**Adicionado em:** 2026-08-25 (Bibble, investigação + Scout + correção direta)
