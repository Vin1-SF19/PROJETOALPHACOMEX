...
**Pendência real, ainda não resolvida:** `Checklist.empresaId → operacional_clientes_old_fase35` (deveria ser `operacional_clientes`) continua pendente — ao investigar, encontrou-se o MESMO padrão de "empresa nunca migrada" já visto em Extratos: **3 empresas** (`ADICEL - INDUSTRIA E COMERCIO LTDA`, `ARCOS DOURADOS COMERCIO DE ALIMENTOS SA`, e novamente `ALPHA COMEX BRASIL LTDA`) existem em `operacional_clientes_old_fase35` mas nunca ganharam registro na tabela `operacional_clientes` atual — cada uma com 1 `Checklist` vinculado (dado real). Complicador adicional: o campo `clienteId` mudou de significado entre as duas tabelas — na antiga aponta para `ClienteOperacional` (conta de login do portal), na nova há DOIS campos (`clienteOperacionalId` = login, `clienteId` = empresa em `Cliente`/módulo master) — não é um rename 1:1 simples como os outros casos. A Alpha Comex já tem `Cliente.id: 288` (criado na correção de Extratos); ADICEL e ARCOS DOURADOS não têm `Cliente` nenhum ainda. Correção requer decisão do usuário sobre como resolver esse caso específico antes de prosseguir — não decidir sozinho.
