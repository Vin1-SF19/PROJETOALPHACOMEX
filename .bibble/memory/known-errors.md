...
**Fix:** quando `npm run build` em background fica sem output por muito mais tempo que o padrão histórico (~15-20min neste projeto), NÃO assumir que morreu e relançar — isso causa exatamente o erro de lock acima. Em vez disso: (1) redirecionar para arquivo com `> log 2>&1; echo EXIT $? >> log` (mais confiável que pipe para `tail` em background); (2) `Get-CimInstance Win32_Process -Filter "Name = 'node.exe'"` SEM filtro de `CommandLine` primeiro, olhar todos os processos por memória/CPU decrescente — o processo certo é o de maior memória, mesmo que o `CommandLine` não bata com o filtro esperado na primeira tentativa; (3) se `.next/lock` existir e outro build for necessário, aguardar o processo original terminar (ele libera o lock sozinho) em vez de forçar um segundo build concorrente.
**Adicionado em:** 2026-08-26 (Bibble, build de validação do RM-2026-2C7A4B — Kanban de Relacionamento de Parceiros)

---

### 22 testes pré-existentes em `tests/bpm/*` falhando por mock incompleto de `@/lib/bpm/automacoes` — não relacionado a mudanças recentes
**Sintoma:** `npx vitest run tests/bpm` falha em ~11 arquivos (`card-modal-integration`, `fechado-actions`, `fechado-ui`, `formulario-etapa`, `lost-actions`, `lost-ui`, `membros-card-ui`, `perfil-empresa-modal`, `prazo-anotacao-card`, `sem-viabilidade-actions`, `standby-follow-up`) com erros como `No "executarAutomacaoTarefaNotaFiscal" export is defined on the "@/lib/bpm/automacoes" mock` e asserções de conteúdo de arquivo (`readFileSync` + `toContain`) que não batem mais com o código-fonte atual.
**Causa raiz confirmada:** NÃO é causado por mudanças da sessão atual — `git status`/`git diff --stat` confirmaram que nenhum desses arquivos-fonte (`Cards.ts` fora das 4 linhas de uma mudança pontual, `automacoes.ts`, componentes de UI do CRM) estava no escopo das edições feitas. É quebra pré-existente no repositório, provavelmente por dessincronia entre testes e código-fonte do BPM ao longo de sessões anteriores (possivelmente da sessão concorrente `painelalpha-48`, que edita arquivos de `CalendarioAlpha`/BPM em paralelo).
**Fix:** ao rodar a suíte completa de `tests/bpm`, NÃO assumir que uma mudança pontual (ex: em `ownership.ts`/`Cards.ts`/`validations/bpm.ts`) causou falhas nesses arquivos sem antes confirmar via `git diff --stat` que o arquivo-fonte relevante realmente foi tocado. Rodar os testes do escopo específico da mudança (ex: `criar-card-nova-empresa.test.ts`, `ownership-security.test.ts`) em vez de toda a pasta `tests/bpm` para validar uma feature nova — a suíte completa já estava quebrada antes.
**Adicionado em:** 2026-08-26 (Bibble, RM-2026-97934A — formulário de indicação de cliente com serviço/pipeline automático)
...
## Objetivo grande do Roadmap Alpha truncava consistentemente ao ser desarquivado — reproduzido de novo em RM-2026-999766
**Sintoma:** objetivo `RM-2026-999766` ("Gerador de Documentos", `isNewModule: true`, description/desiredOutcome/constraints extensos + 13 critérios de aceite) estava `ARCHIVED`. Ao ser desarquivado manualmente (reset de `archivedAt`, `status=ACTIVE`, `documentationStatus=PENDING`, nova `sourceVersion`, novo `RoadmapDocumentationJob`), o worker reivindicou o job e falhou 3x seguidas com `TRUNCATED_MODEL_RESPONSE` (tentativas 1, 2 e 3, mesmo padrão descrito no comentário de `qwen-generator.ts:241-243`) — confirma que esse objetivo provavelmente foi arquivado da primeira vez por esse mesmo motivo, não por decisão de produto.
**Causa:** mesma já documentada em `qwen-generator.ts` — `num_ctx` fixo em 32_768, prompt de sistema (protocolo Bibble + catálogo de agentes + contexto do projeto) consome a maior parte do budget, sobrando pouco para o manifesto de saída de objetivos com texto de entrada muito longo. Comportamento **100% consistente** — não adianta deixar o worker retentar até `maxAttempts`, vai sempre truncar.
**Mitigação aplicada:** job cancelado manualmente (`roadmapDocumentationJob.status → CANCELLED`, `objective.documentationStatus → FAILED`) em vez de deixar rodar as 30 tentativas. Blueprint de integração feito manualmente por Bibble assumindo o papel do Scout (leitura direta do codebase), sem depender do worker Qwen automático — permite prosseguir com objetivos grandes sem precisar arquivá-los.
**Desarquivar um objetivo (não existe função pronta):** replicar manualmente a lógica de `retryRoadmapObjective` (`src/lib/roadmap-alpha/objectives.ts:155`) + zerar `archivedAt` + `globalPriority=1` (topo da fila) numa única transação — `createRoadmapObjective`/`retryRoadmapObjective`/`moveRoadmapObjective` todos rejeitam objetivo com `archivedAt != null` (`OBJECTIVE_NOT_FOUND`), e não há Server Action de "desarquivar" na UI.
