...
- Exclusão definitiva de campo com dados (RM-2026-76D6E4, 2026-09-24): `ExcluirCampoBpm` em `src/actions/bpm/Campos.ts` deixa de bloquear quando o campo tem valores/anexos; exige `confirmarDescarteDados: true` + `usoConfirmado` (cinco contagens) revalidados na transação (`USO_CAMPO_ALTERADO`/`CONFIRMACAO_DESCARTE_OBRIGATORIA`). Apaga anexos (`SetNull`) e mapeamentos (`Restrict`) explicitamente; valores, valores globais, componentes de formulário, opções, acessos e configurações de etapa saem por cascade. Limpeza de blobs privados pós-commit via `limparBlobAnexoPendente`/reconciliador; auditoria `campo_excluido`. Schema `excluirCampoSchema` em `src/lib/validations/bpm.ts` (`.strict()` em `usoConfirmado`). UI em `FormularioEtapaWorkspace.tsx` (diálogo "Excluir campo aplicável" com contagens + "Excluir definitivamente"). Sem migration. Testes: `tests/bpm/{campos-configuraveis-actions,pipeline-editor-react}.test.ts`.
- Ressalva verificada em 2026-09-24 por Scribe (RM-2026-8996E2): o código atual de `CardModal/CardFullViewModal.tsx` aguarda `flushSaves()` e mantém “Campos não salvos”, divergindo da descrição histórica RM-2026-B88712 abaixo. `fechando` sinaliza essa espera; `pipeline/[pipelineId]/PipelineBoardClient.tsx` usa `cardMovendoId` para o overlay pós-drop. Nenhuma estrutura ou rota nova. Evidências e pendências: `docs/stories/story-rm-2026-8996e2-respostas-visuais-loading.md`.
- Autosave universal do card CRM (RM-2026-B88712, 2026-09-22): `CardSaveProvider` em `src/app/PainelAlpha/AlphaCRM/layout.tsx` (acima do modal, sobrevive ao fechamento). Hook `useCardSave` em `CardModal/CardSaveContext.tsx` expõe `scheduleSave(key, cb, delay=500)`, `registerSave(cb, cardId, recoveryKey)`, `flushScheduled(prefix)`, `flushSaves(cardId)`, `getDraft`/`setDraft`, `getVersion`, `subscribeConfirmation`. Consumidores: `PainelCamposEtapaAtual`, `PainelProximoContato`, `PainelReuniao`, `PainelChecklistFollowUp`, `PainelStatusPosFechamento`, `PainelProximaEtapa`, `CampoBpmInput` (upload). Diálogo "Campos não salvos" removido de `CardFullViewModal.tsx`. Testes: `tests/bpm/{cpf-fechamento,cpf-pendencias,autosave-tipos-imediatos,autosave-recovery,autosave-fixed-recovery,card-save-flow,edicao-campos-card,arquivo-persistencia}-react.test.ts`.
- Bibble Voice local (atualizado em 2026-09-18): o Painel mantém chat/LLM/STT existentes e acrescenta TTS por `src/app/api/bibble/voice/route.ts`, com contrato server-only em `src/lib/bibble/voice-{service,admission,client,preferences}.ts` e reprodução em `src/components/BibbleChatHome/{BotaoFalarMensagem,bibble-audio-manager,useVoiceStatus}.ts(x)`. O navegador segue `browser → /api/bibble/voice → BIBBLE_VOICE_URL → /v1/speech`; a rota ativa responde 401 sem sessão, em vez de 404. No POST, o guard compara `Origin` com o header `Host` público, não com a URL interna da request, preservando compatibilidade com Cloudflare Tunnel sem abrir cross-site: ausência, formato inválido, divergência e `Sec-Fetch-Site` incompatível continuam bloqueados. O microserviço independente em `/home/ialpha/services/bibble-voice` usa FastAPI/Chatterbox Multilingual V3, `TTS_DEVICE=cpu|cuda`, cache WAV PCM16, fila serializada, carga sob demanda e unload após 300 s. `app/text_normalization.py` expande datas standalone `dd/mm/aaaa` e `dd-mm-aaaa` válidas para cardinal pt-BR; `app/engine.py` aplica obrigatoriamente `sanitize_text → normalize_spoken_dates → cache key → split/model`, fazendo slash e hífen convergirem para o mesmo WAV sem alterar datas inválidas nem tokens lexicais protegidos. A operação atual usa CPU/8 threads e referência autorizada `voices/bibble.wav`; testes reais comprovaram MISS, HIT, unload, restart fail-stop para devolver RAM e reload com novo MISS. A GPU/llama.cpp permaneceu intacta.
- Bibble/IAlpha — abertura de chamado condicionada (atualizado em 2026-09-17): `abrir_chamado` é a única tool mutável publicada por `src/lib/bibble/tools.ts`; `src/lib/bibble/chamado-guard.ts` a seleciona somente diante de pedido explícito no turno atual, enquanto consultas continuam separadas em `consultar_chamados`. A rota emite em `src/lib/bibble/mutation-grant.ts` um grant opaco `WeakMap`, de uso único e vinculado a usuário, request, tool, texto original e TTL máximo de 120 s. `src/lib/bibble/tool-executor.ts` exige esse grant, valida payload Zod estrito e literalidade contra o pedido, resolve responsável de TI ativo por nome e grava em transação `Serializable`, revalidando solicitante, técnico e deduplicação de cinco minutos antes do create. Criar chamado próprio tem metadata `permission: null` para qualquer usuário autenticado; consultar chamados continua exigindo `chamados`. Não houve schema/migration. O smoke concorrente no Turso real permanece pendente.
...
- Checklist Builder: models Prisma `BpmChecklistTemplate*`/`BpmChecklistTemplateEtapa`/`BpmCardChecklist*`; domínio em `src/lib/bpm/checklists/{schemas,leitura,service,integracao}.ts` (`leitura.ts` expõe `filtroEtapaTemplateChecklist`, fonte única de aplicabilidade de etapa, RM-2026-457A31); actions em `src/actions/bpm/Checklists.ts` (inclui `ExcluirTemplateChecklistBpm` — soft delete `ativo:false` + auditoria `checklist_template_excluido`, RM-2026-E96332); workspace em `src/components/bpm/checklists/{ChecklistsWorkspace,EtapasMultiSelect}.tsx` + `checklist-editor-state.ts` (multiselect de etapas, RM-2026-457A31; botão `Trash2` + `Dialog` de confirmação com bloqueio Esc/clique fora e aviso "em uso por N cards", RM-2026-E96332); painel em `CardModal/PainelChecklistsCard.tsx`, montado em `PainelHistorico.tsx` (aba Checklist); alerta/navegação em `PainelProximaEtapa.tsx` + `PainelRegistrar.tsx`; ação automática `MATERIALIZAR_CHECKLIST` em `src/lib/bpm/automacoes/{schemas,executor}.ts`.
...
> Última atualização: 2026-09-18 por Scribe (normalização de datas do Bibble Voice antes do cache/modelo/split)
...
| Serviço local | `/home/ialpha/services/bibble-voice/app/{main,engine,cache,schemas,config,text_normalization}.py` | FastAPI, Chatterbox Multilingual V3 em CPU ou CUDA explícita, normalização segura de datas antes do cache/modelo/split, cache WAV PCM16, fila, timeout, observabilidade e descarga de recursos |
...
Fluxo: a resposta textual continua sendo produzida e persistida pelo pipeline existente. Para uma mensagem nativa elegível, o botão chama `POST /api/bibble/voice`; essa rota autentica a sessão, aplica limite por usuário/global e encaminha server-to-server para `${BIBBLE_VOICE_URL}/v1/speech`. O FastAPI aceita apenas `voice="bibble"` e `language="pt"`, resolve internamente `voices/bibble.wav` e processa o texto nesta ordem obrigatória: sanitiza HTML/código/URLs, expande datas brasileiras standalone válidas, calcula a chave do cache e só então divide/entrega ao modelo. Em miss, serializa a geração no dispositivo escolhido explicitamente por `TTS_DEVICE`. O WAV PCM signed de 16 bits volta para a mesma bolha; nenhuma mensagem adicional é criada.
...
Estado operacional conferido em 2026-09-18: `bibble-voice.service` foi reiniciado de forma controlada e permanece habilitado e ativo em `127.0.0.1:8787`, com health local `ok`, smoke público aprovado, `device=cpu` e referência autorizada configurada. A regressão Python soma 64 testes aprovados e comprova que slash/hífen convergem para a mesma chave de cache. `TTS_CPU_THREADS=8` limita PyTorch; a unidade usa `Nice=10`, `CPUWeight=50`, `MemoryHigh=8G` e `MemoryMax=12G`. Em CPU, unload faz `del`/GC e agenda encerramento fail-stop do próprio worker; o systemd reinicia somente o serviço de voz para devolver RAM ao sistema. O smoke autenticado completo da UI e o AC20 continuam pendentes, sem inferir aceite ponta a ponta no navegador.
...
| `src/app/PainelAlpha/AlphaCRM/CardModal/CardFullViewModal.tsx` | Consumidor do provider no layout CRM; blur + flush ao fechar (RM-2026-B88712, 2026-09-22) |
...
O catálogo público atual contém 20 tools: 19 somente leitura e a única mutação `abrir_chamado`, condicionada a intenção explícita e grant server-owned de uso único. Filesystem e todas as demais mutações foram removidos do provider e falham fechados no executor. Upload/anexos também permanecem indisponíveis (`503`) até existir storage privado com ownership verificável; a UI não oferece essa capacidade enquanto a contenção estiver ativa.

### Abertura de chamado pelo Bibble — mutação condicionada (2026-09-17)

- `src/lib/bibble/chamado-guard.ts` reconhece somente pedidos diretos atuais de abrir/criar/registrar/fazer chamado; consulta, negação, pergunta de instrução, texto citado e relato em terceira pessoa não concedem a capacidade.
- `src/lib/bibble/tool-policy.ts` publica `abrir_chamado` para qualquer usuário autenticado com metadata `permission: null`, porque a abertura própria segue o fluxo manual; `consultar_chamados` permanece separado e exige a permissão `chamados`.
- `src/app/api/bibble/chat/route.ts` emite o grant apenas quando o detector confirmou intenção, a tool está autorizada naquele turno, não há anexos e o deadline ainda é válido. `src/lib/bibble/mutation-grant.ts` guarda estado em `WeakMap`, sem serializar segredo para modelo/cliente, e consome a referência uma única vez mesmo quando usuário, request ou tool não correspondem. O vínculo inclui usuário, request, nome da tool, texto original autorizado e TTL de no máximo 120 segundos.
- `src/lib/bibble/tool-executor.ts` exige o grant para a mutação, usa Zod estrito para título, descrição, prioridade e nome opcional do responsável, e aceita somente campos literais derivados da mensagem autorizada. O responsável é resolvido por nome entre usuários `ATIVO` de TI, sem aceitar ID gerado pelo modelo.
- A persistência ocorre em transação `Serializable`: revalida solicitante ativo, deduplica mesmo usuário+título na janela de cinco minutos, revalida técnico ativo/elegível e só então cria o chamado com `status: "ABERTO"`, categoria `Outro` e `tecnicoSolicitadoId` opcional. Falha/ambiguidade não deve ser apresentada como sucesso; o guard da resposta exige o marcador real `SUCESSO_ABRIR_CHAMADO`.
- Cobertura dedicada: `tests/bibble/chamado-creation-security.test.ts`, `tests/bibble/route-runner.integration.test.ts` e casos de intenção/política em `tests/bibble/hardening-and-persona.test.ts`.
- Não houve schema, migration, seed ou backfill. O smoke concorrente contra o Turso real continua pendente; testes locais/mocks não equivalem a essa comprovação operacional.

**Última atualização:** 2026-09-17 por Scribe (abertura de chamado condicionada; smoke concorrente Turso pendente)
...
## Estoque Geral do Painel Alpha (2026-09-18)

- `src/app/PainelAlpha/Estoque/page.tsx` é a única composição canônica; a URL antiga de Serviços Gerais executa redirect server-side.
- `src/lib/estoque/` concentra autorização, domínio, schemas, serviços transacionais de item/movimento/kit, consultas e tratamento seguro de imagens.
- `src/actions/{Estoque,EstoqueMovimentos,EstoqueKits}.ts` são adaptadores autenticados; `scripts/inventory/cli.ts` reutiliza os mesmos serviços e permissões efetivas.
- `src/components/Estoque/` contém itens, detalhes, movimentos, atribuições, manutenção, categorias, Tags/modelos e instâncias de kit.
- `InventoryOperation`, `InventoryMovement`, `InventoryBalance`, `InventoryAsset`, `InventoryAssignment`, `InventoryMaintenance`, `InventoryTag*`, `InventoryKit*` e `InventoryAuditLog` formam o domínio aditivo. `ProdutoEstoque` e `Categoria` continuam raízes compatíveis; `ListaCompra` não recebeu FK nem alteração estrutural.
- Artefatos SQL autorizados: baseline legado, foundation V1/V2 e backfill V3. Não existem V4/V5/V6 autorizadas.

**Última atualização:** 2026-09-18 por Codex (Estoque Geral)

### 2026-09-22 — Scribe — RM-2026-B88712: estrutura do autosave CRM

A fronteira de estado mudou: `src/app/PainelAlpha/AlphaCRM/layout.tsx` hospeda `CardModal/CardSaveContext.tsx` acima de DashboardClient, PipelineBoardClient e TarefasCentralClient. `CardFullViewModal.tsx` consome o provider e dispara flush; não o instancia. `PainelCamposEtapaAtual.tsx` agenda os campos e apresenta o indicador inline de salvamento. Não houve nova rota nem novo componente de status exportado.

Servidor: `src/actions/bpm/Cards.ts` + `src/lib/validations/bpm.ts` (valores); `src/actions/bpm/Anexos.ts`, `src/app/api/bpm/upload/route.ts` e `src/lib/bpm/upload-conteudo.ts` (upload validado); download existente em `src/app/api/bpm/anexos/[anexoId]/route.ts`. Testes React de autosave/recuperação em `tests/bpm/`. Story canônica: `docs/stories/story-rm-2026-b88712-autosave-card-crm.md`.


### RM-2026-04A236 — Fase 5 Nova

RM-2026-04A236: ListaCamposFormulario.tsx no diretório administrativo de pipelines renderiza a lista plana; src/lib/bpm/ordem-formulario.ts projeta/move componentes preservando seções e identidades.
