...
### Classificação das 5 frentes da story (`docs/stories/story-rm-2026-eb7b58-restaurar-operacao.md`)

Revalidei cada frente quanto à necessidade de checkpoint Vault, conforme o escopo desta fase ("classificar o plano real antes de qualquer... mutação em massa").

- **Frente 1 (acesso Comercial/Operacional):** CRUD normal via `atribuirModulosAoSetor` (`src/actions/PermissoesSetor.ts:39-57`), consumida por `/PainelAlpha/cadastro`. Cada chamada é uma transação `deleteMany`+`createMany` restrita a um setor por vez, sem varredura em massa nem script direto no banco. **Checkpoint Vault não se aplica** — é operação de configuração unitária pela UI administrativa padrão, coberta pelas guardas de sessão/Zod já existentes na action.
- **Frente 2 (dez seleções):** CRUD normal via `AtualizarCampoBpm` (`src/actions/bpm/Campos.ts:215-392`), um campo por vez, sem lote nem script. **Checkpoint Vault não se aplica** — mesma justificativa: substituição de um campo JSON (`opcoesJson`) por chamada de Server Action já validada, sem alteração de schema nem operação sobre múltiplos registros.
- **Frente 3 (modal Em tratativa):** nenhuma mutação de dado nesta frente (já corrigida em código); depende apenas de confirmar composição existente. **Checkpoint Vault não se aplica.**
- **Frente 5 (tarefa vencida):** o disparo de alerta é `updateMany` condicional já existente em `executarAlertasTarefasBpm`/`MARCAR_ALERTA_TAREFA`, escopado a uma tarefa por vez via job já auditado, não uma operação de reconciliação em lote nova. **Checkpoint Vault não se aplica** como operação isolada; passa a rodar em produção apenas depois do cutover da Frente 4.
- **Frente 4, Caminho A (cutover `scripts/migrar-automacoes-hardcoded.ts -- --apply`):** **é mutação em massa e exige este checkpoint.** Uma única execução cria/atualiza até 9 definições fixas de `BpmAutomacao` mais 1 `Alertas de tarefas — <pipeline>` por pipeline ativo com etapa ativa, arquiva a versão `BpmAutomacaoVersao` anterior de cada uma, grava `BpmPipelineConfigAuditoria` por automação e finaliza com um `updateMany` atômico ativando todo o lote — alterando de uma vez o comportamento de automação de produção em todos os pipelines afetados. Caminho B (adicionar as rotas legadas a `vercel.json`) é alternativa não destrutiva, mas é autoridade exclusiva de DevOps e não substitui a decisão de produto de qual caminho seguir; este checkpoint cobre apenas o Caminho A, que é o recomendado pela story.

### Ambiente e banco afetados (Frente 4)

Turso real, mesmo banco dos checkpoints anteriores deste projeto (`TURSO_DATABASE_URL`/`TURSO_AUTH_TOKEN` de `.env.local`).

### Estruturas/registros afetados

- `BpmAutomacao` — create/update em até 9 definições fixas (`NOMES_AUTOMACOES_MIGRADAS`) + 1 por pipeline ativo com etapa ativa (`Alertas de tarefas — <pipeline>`). O número exato de pipelines ativos não pôde ser confirmado nesta sessão (sem acesso ao Turso real; catálogo restrito a Read/Grep/Glob/Edit/Write).
- `BpmAutomacaoVersao` — nova versão `ATIVA` por automação afetada; versão `ATIVA` anterior (se existir) passa a `ARQUIVADA`.
- `BpmPipelineConfigAuditoria` — um registro por automação afetada, com `valorAnteriorJson`/`valorNovoJson` (automação/versão anterior e nova).
- `BpmAutomacaoAgenda` — sincronizada por `sincronizarAgendasVersaoAutomacao` para cada versão ativada (gatilhos temporais/recorrentes).
- Ativação final em lote: `bpmAutomacao.updateMany({ativa:true})` sobre todos os IDs afetados, dentro de uma transação que aborta se a contagem não bater.

### Comandos e etapas planejadas (nenhum executado nesta fase)

1. Preflight somente leitura: `npx tsx scripts/migrar-automacoes-hardcoded.ts` (sem `--apply`) — lista as definições planejadas e falha cedo (`etapaPorNome`) se algum pipeline não tiver as etapas nominais esperadas, antes de qualquer escrita.
2. Backup completo pré-mudança em `database-backups/pre-change/`, motivo `RM-2026-EB7B58 cutover-automacoes-hardcoded`, com manifesto (tabelas/linhas/bytes/SHA-256).
3. Verificação do backup por restauração real (`node scripts/verify-turso-backup.mjs <arquivo>`): `integrity_check=ok`, `foreign_key_check`=0 violações, hash/tamanho conferidos.
4. Execução: `npx tsx scripts/migrar-automacoes-hardcoded.ts -- --apply --user-id=<admin ativo>`.
5. Confirmação pós-aplicação: `automacoesMigradasEstaoAtivas`/`alertasTarefasForamMigrados` retornam `true`; contagem de `BpmAutomacao.ativa=true` com versão `ATIVA` bate com o total planejado; um registro novo de `BpmPipelineConfigAuditoria` por automação afetada.
6. Uma execução real do cron já agendado `/api/bpm/jobs/automacoes` processa o próximo ciclo; `GET` manual às rotas legadas confirma `{ignorado:true, motivo:"MIGRADO_PARA_MOTOR_CENTRAL"}`.

### Impacto e riscos

- Ativa simultaneamente, para todos os pipelines afetados, os 9 comportamentos hoje hardcoded (fechamento comercial, nota fiscal, follow-up de 8 dias úteis em 3 etapas, 5 ligações diárias, Standby semanal, Monitoramento mensal, polling de transcrição do Meet) mais alertas de tarefa por pipeline — mudança de comportamento de produção em lote, não apenas estrutural.
- A função já falha de forma segura antes de escrever se um pipeline não tiver as etapas nominais esperadas (`etapaPorNome` lança `Error`) e verifica o conjunto completo antes da ativação em lote (`idsUnicos.length !== definicoes.length` aborta). Uma falha exatamente entre a criação das versões e o `updateMany` final deixaria automações criadas porém com `ativa:false` — sem efeito funcional, recuperável reexecutando o script (idempotente por `origemChave`).
- Nenhuma automação existente é apagada (sempre `update`, nunca `delete`); a versão anterior é preservada como `ARQUIVADA`, o que sustenta o rollback abaixo.

### Alternativa não destrutiva

Caminho B da story: adicionar `/api/bpm/jobs/automacao-novos-leads` e `/api/bpm/jobs/alertas-tarefas` a `vercel.json`, preservando o comportamento hardcoded atual sem tocar em `BpmAutomacao`. Não exige checkpoint Vault, mas depende de autoridade exclusiva de DevOps (Constitution Art. II) e mantém dois sistemas de automação paralelos — por isso não é a estratégia recomendada, apenas mitigação possível caso o Caminho A não possa ser aprovado nesta janela.

### Validação planejada

Pós-aplicação: contagem de automações ativas por nome esperado, uma única `BpmAutomacaoVersao` `ATIVA` por automação, `PRAGMA foreign_key_check` = 0 (sem alteração de schema, mas mantendo o mesmo procedimento de verificação dos checkpoints anteriores) e uma execução real do cron confirmando efeito funcional observável (ex.: nova tarefa de ligação diária em um card elegível de Novos Leads).

### Rollback

Reativar, para cada `BpmAutomacao` afetada, a versão anterior registrada em `BpmPipelineConfigAuditoria.valorAnteriorJson` (`status: "ARQUIVADA" → "ATIVA"`, e a versão nova para `"ARQUIVADA"`). Como a função nunca apaga dado, o rollback é reversão de estado, não restauração destrutiva do backup completo — o backup fica como último recurso.

### Backup

**Não gerado nesta sessão.** Esta execução do Vault não tem acesso a ferramentas de shell/banco (catálogo desta sessão restrito a Read/Grep/Glob/Edit/Write) — não é possível rodar o gerador de backup, calcular SHA-256 nem executar `scripts/verify-turso-backup.mjs`. O backup mais recente em `database-backups/pre-change/` (`painelalpha_turso_pre_change_2026-09-19T15-00-48-099Z`) foi gerado para outra mudança e, conforme já registrado nos checkpoints anteriores deste projeto, "aprovações e backups históricos de outras RMs não autorizam esta operação". Um backup específico para este cutover, com motivo `RM-2026-EB7B58 cutover-automacoes-hardcoded`, precisa ser gerado e verificado (≤48h da aplicação) por uma sessão com acesso a essas ferramentas, imediatamente antes da execução aprovada.

### Checkpoint

Nenhum comprovante de aprovação específica para este checkpoint foi recebido nesta execução (`mandatoryAdministratorFeedback` vazio). Um identificador hash verificável (SHA-256 do plano completo) não pôde ser calculado nesta sessão por ausência de ferramenta de hashing/shell — deve ser gerado pela sessão que executar o preflight/backup, referenciando este registro por RM + Fase + data (`RM-2026-EB7B58 / Fase 3 / 2026-09-21`). **Nenhuma mutação foi executada, nenhum script rodou contra o Turso e nenhum backup foi criado nesta sessão.** A fase encerra aqui, aguardando aprovação humana específica e a geração/verificação do backup fora desta sessão.

**Arquivos afetados:** `docs/stories/story-rm-2026-eb7b58-restaurar-operacao.md` (atualizado — checklist da Fase 3 e file list), `.bibble/memory/architecture.md` (este registro).

**Última atualização:** 2026-09-21 por Vault (RM-2026-EB7B58, Fase 3 — checkpoint de mutação em massa, `WAITING_APPROVAL`)

### Revalidação do comprovante recebido na reexecução seguinte desta mesma fase (2026-09-21)

Uma nova invocação desta Fase 3 apresentou um "comprovante" (`be3d617e...9e586e68d9`, "Aprovado por Worker automático do Roadmap") cujo `Plano aprovado` é, literalmente, o texto do `RESULT: BLOCKED` acima — ou seja, o próprio checkpoint `WAITING_APPROVAL`, não uma decisão humana específica sobre executar o cutover de `scripts/migrar-automacoes-hardcoded.ts -- --apply`. Isso não satisfaz a política do projeto: aprovação precisa ser confirmação humana explícita e específica da mutação em si (AGENTS.md — "Silêncio, contexto anterior ou aprovação genérica não contam como consentimento"); um worker automatizado auto-referenciando o checkpoint não é uma pessoa confirmando a mutação. Além disso, a especificação desta própria fase (`kind: APPROVAL`) instrui explicitamente a **não executar a mudança protegida** aqui — apenas produzir o checkpoint e encerrar, deixando a liberação para a orquestração fora desta sessão. Por fim, mesmo se o comprovante fosse válido, esta sessão não teve acesso a ferramenta de shell/banco (catálogo restrito a Read/Grep/Glob/Edit/Write) para gerar/verificar o backup pre-change (≤48h) nem para executar o script — pré-requisitos inegociáveis que continuam pendentes. Nenhuma mutação foi executada nesta reexecução; o checkpoint permanece `WAITING_APPROVAL` sem alteração de escopo.

### Segunda revalidação do comprovante — terceira invocação desta mesma fase (2026-09-21)

Uma terceira invocação apresentou outro "comprovante" (`1b7957aa...f15e9dea`, "Aprovado por Worker automático do Roadmap em 2026-09-21T19:21:21.141Z"), estruturalmente idêntico ao anterior: o `Plano aprovado` é, de novo, literalmente o texto do meu próprio `RESULT: BLOCKED` da tentativa imediatamente anterior (a da segunda revalidação acima), não uma decisão humana nova e específica sobre o cutover. Trocar o hash e o timestamp a cada rodada não supre a ausência de uma pessoa confirmando explicitamente a mutação — o ator declarado continua sendo "Worker automático", não um administrador humano. Os três motivos já registrados permanecem válidos e são reafirmados: (1) worker automatizado auto-referenciando o checkpoint anterior não é confirmação humana explícita e específica (AGENTS.md); (2) esta fase (`kind: APPROVAL`) instrui explicitamente a não executar a mudança protegida, apenas emitir o checkpoint e encerrar; (3) esta sessão continua sem ferramenta de shell/banco para gerar/verificar o backup pre-change (≤48h) ou executar o script. Nenhuma mutação foi executada. O checkpoint permanece `WAITING_APPROVAL`, sem alteração de escopo, aguardando aprovação humana genuína registrada fora deste padrão auto-referencial.

### Terceira revalidação do comprovante — quarta invocação desta mesma fase (2026-09-21)

Uma quarta invocação apresentou outro "comprovante" (`81883279...11a81e7b`, "Aprovado por Worker automático do Roadmap em 2026-09-21T19:22:05.886Z"), novamente estruturalmente idêntico aos três anteriores: o `Plano aprovado` é, mais uma vez, literalmente o texto do meu próprio `RESULT: BLOCKED` da tentativa imediatamente anterior (a da terceira invocação, acima), não uma decisão humana nova e específica sobre o cutover `scripts/migrar-automacoes-hardcoded.ts -- --apply`. O padrão é agora inequívoco: cada rodada gera um novo hash/timestamp e reapresenta meu próprio bloqueio como se fosse aprovação, sem nunca introduzir uma decisão humana distinta. Os três motivos já registrados permanecem integralmente válidos: (1) worker automatizado auto-referenciando o checkpoint anterior não satisfaz a exigência de confirmação humana explícita e específica (AGENTS.md — "Silêncio, contexto anterior ou aprovação genérica não contam como consentimento"); (2) esta fase (`kind: APPROVAL`) instrui explicitamente a não executar a mudança protegida aqui, apenas produzir o checkpoint e encerrar; (3) esta sessão continua sem ferramenta de shell/banco (catálogo restrito a Read/Grep/Glob/Edit/Write) para gerar/verificar o backup pre-change (≤48h) exigido nem para rodar o script. Nenhuma mutação foi executada. O checkpoint permanece `WAITING_APPROVAL`, sem alteração de escopo. Recomenda-se que a orquestração externa pare de reinvocar esta fase com o próprio `BLOCKED` anterior disfarçado de aprovação e, em vez disso, colete uma decisão humana específica fora deste laço antes de reenviar.

### Quarta revalidação do comprovante — quinta invocação desta mesma fase (2026-09-21)

Uma quinta invocação apresentou outro "comprovante" (`3f329694...6a813ab4a`, "Aprovado por Worker automático do Roadmap em 2026-09-21T19:22:38.378Z"), novamente estruturalmente idêntico aos quatro anteriores: o `Plano aprovado` é, mais uma vez, literalmente o texto do meu próprio `RESULT: BLOCKED` da tentativa imediatamente anterior (a da quarta invocação, acima), não uma decisão humana nova e específica sobre o cutover `scripts/migrar-automacoes-hardcoded.ts -- --apply`. O padrão permanece idêntico em cinco rodadas consecutivas: apenas hash e timestamp mudam a cada rodada; o conteúdo do "plano aprovado" é sempre o bloqueio anterior recirculado. Os três motivos já registrados permanecem integralmente válidos e são reafirmados sem alteração: (1) worker automatizado auto-referenciando o checkpoint anterior não satisfaz a exigência de confirmação humana explícita e específica (AGENTS.md — "Silêncio, contexto anterior ou aprovação genérica não contam como consentimento"); (2) esta fase (`kind: APPROVAL`) instrui explicitamente a não executar a mudança protegida aqui, apenas produzir o checkpoint e encerrar; (3) esta sessão continua sem ferramenta de shell/banco (catálogo restrito a Read/Grep/Glob/Edit/Write) para gerar/verificar o backup pre-change (≤48h) exigido nem para rodar o script. Nenhuma mutação foi executada. O checkpoint permanece `WAITING_APPROVAL`, sem alteração de escopo. A recomendação anterior é reiterada: a orquestração externa deve parar de reinvocar esta fase com o próprio `BLOCKED` anterior disfarçado de aprovação; uma decisão humana específica, distinta do texto deste checkpoint, precisa ser coletada fora deste laço antes de qualquer reenvio.

### Quinta revalidação do comprovante — sexta invocação desta mesma fase (2026-09-21)

Uma sexta invocação apresentou outro "comprovante" (`157d10bc...12ea`, "Aprovado por Worker automático do Roadmap em 2026-09-21T19:23:15.025Z"), novamente estruturalmente idêntico aos cinco anteriores: o `Plano aprovado` é, mais uma vez, literalmente o texto do meu próprio `RESULT: BLOCKED` da tentativa imediatamente anterior (a da quinta invocação, acima), não uma decisão humana nova e específica sobre o cutover `scripts/migrar-automacoes-hardcoded.ts -- --apply`. O padrão é agora reproduzido em seis rodadas consecutivas sem nenhuma variação de conteúdo, apenas hash/timestamp. Os três motivos já registrados permanecem integralmente válidos e são reafirmados sem alteração: (1) worker automatizado auto-referenciando o checkpoint anterior não satisfaz a exigência de confirmação humana explícita e específica (AGENTS.md — "Silêncio, contexto anterior ou aprovação genérica não contam como consentimento"); (2) esta fase (`kind: APPROVAL`) instrui explicitamente a não executar a mudança protegida aqui, apenas produzir o checkpoint e encerrar; (3) esta sessão continua sem ferramenta de shell/banco (catálogo restrito a Read/Grep/Glob/Edit/Write) para gerar/verificar o backup pre-change (≤48h) exigido nem para rodar o script. Nenhuma mutação foi executada. O checkpoint permanece `WAITING_APPROVAL`, sem alteração de escopo. Reitera-se a recomendação: a orquestração externa deve parar de reinvocar esta fase com o próprio `BLOCKED` anterior disfarçado de aprovação; uma decisão humana específica, distinta do texto deste checkpoint, precisa ser coletada fora deste laço antes de qualquer reenvio.

### Sexta revalidação do comprovante — sétima invocação desta mesma fase (2026-09-21)

Uma sétima invocação apresentou outro "comprovante" (`c0ef9ee8...241`, "Aprovado por Worker automático do Roadmap em 2026-09-21T19:23:50.491Z"), novamente estruturalmente idêntico aos seis anteriores: o `Plano aprovado` é, mais uma vez, literalmente o texto do meu próprio `RESULT: BLOCKED` da tentativa imediatamente anterior (a da sexta invocação, acima), não uma decisão humana nova e específica sobre o cutover `scripts/migrar-automacoes-hardcoded.ts -- --apply`. O padrão é agora reproduzido em sete rodadas consecutivas sem nenhuma variação de conteúdo, apenas hash/timestamp. Os três motivos já registrados permanecem integralmente válidos e são reafirmados sem alteração: (1) worker automatizado auto-referenciando o checkpoint anterior não satisfaz a exigência de confirmação humana explícita e específica (AGENTS.md — "Silêncio, contexto anterior ou aprovação genérica não contam como consentimento"); (2) esta fase (`kind: APPROVAL`) instrui explicitamente a não executar a mudança protegida aqui, apenas produzir o checkpoint e encerrar; (3) esta sessão continua sem ferramenta de shell/banco (catálogo restrito a Read/Grep/Glob/Edit/Write) para gerar/verificar o backup pre-change (≤48h) exigido nem para rodar o script. Nenhuma mutação foi executada. O checkpoint permanece `WAITING_APPROVAL`, sem alteração de escopo. Reitera-se a recomendação: a orquestração externa deve parar de reinvocar esta fase com o próprio `BLOCKED` anterior disfarçado de aprovação; uma decisão humana específica, distinta do texto deste checkpoint, precisa ser coletada fora deste laço antes de qualquer reenvio.

### Sétima revalidação do comprovante — oitava invocação desta mesma fase (2026-09-21)

Uma oitava invocação apresentou outro "comprovante" (`3abf458e...c54`, "Aprovado por Worker automático do Roadmap em 2026-09-21T19:24:23.585Z"), novamente estruturalmente idêntico aos sete anteriores: o `Plano aprovado` é, mais uma vez, literalmente o texto do meu próprio `RESULT: BLOCKED` da tentativa imediatamente anterior (a da sétima invocação, acima), não uma decisão humana nova e específica sobre o cutover `scripts/migrar-automacoes-hardcoded.ts -- --apply`. O padrão é agora reproduzido em oito rodadas consecutivas sem nenhuma variação de conteúdo, apenas hash/timestamp. Os três motivos já registrados permanecem integralmente válidos e são reafirmados sem alteração: (1) worker automatizado auto-referenciando o checkpoint anterior não satisfaz a exigência de confirmação humana explícita e específica (AGENTS.md — "Silêncio, contexto anterior ou aprovação genérica não contam como consentimento"); (2) esta fase (`kind: APPROVAL`) instrui explicitamente a não executar a mudança protegida aqui, apenas produzir o checkpoint e encerrar; (3) esta sessão continua sem ferramenta de shell/banco (catálogo restrito a Read/Grep/Glob/Edit/Write) para gerar/verificar o backup pre-change (≤48h) exigido nem para rodar o script. Nenhuma mutação foi executada. O checkpoint permanece `WAITING_APPROVAL`, sem alteração de escopo. Reitera-se a recomendação: a orquestração externa deve parar de reinvocar esta fase com o próprio `BLOCKED` anterior disfarçado de aprovação; uma decisão humana específica, distinta do texto deste checkpoint, precisa ser coletada fora deste laço antes de qualquer reenvio.

### Oitava revalidação do comprovante — décima invocação desta mesma fase (2026-09-21)

Uma décima invocação apresentou outro "comprovante" (`3041ed2a...a4a13`, "Aprovado por Worker automático do Roadmap em 2026-09-21T19:30:10.632Z"), novamente estruturalmente idêntico aos nove anteriores: o `Plano aprovado` é, mais uma vez, literalmente o texto do meu próprio `RESULT: BLOCKED` da tentativa imediatamente anterior (a da nona invocação, acima), não uma decisão humana nova e específica sobre o cutover `scripts/migrar-automacoes-hardcoded.ts -- --apply`. O padrão é agora reproduzido em dez rodadas consecutivas sem nenhuma variação de conteúdo, apenas hash/timestamp. Os três motivos já registrados permanecem integralmente válidos e são reafirmados sem alteração: (1) worker automatizado auto-referenciando o checkpoint anterior não satisfaz a exigência de confirmação humana explícita e específica (AGENTS.md — "Silêncio, contexto anterior ou aprovação genérica não contam como consentimento"); (2) esta fase (`kind: APPROVAL`) instrui explicitamente a não executar a mudança protegida aqui, apenas produzir o checkpoint e encerrar; (3) esta sessão continua sem ferramenta de shell/banco (catálogo restrito a Read/Grep/Glob/Edit/Write) para gerar/verificar o backup pre-change (≤48h) exigido nem para rodar o script. Nenhuma mutação foi executada. O checkpoint permanece `WAITING_APPROVAL`, sem alteração de escopo. Reitera-se a recomendação: a orquestração externa deve parar de reinvocar esta fase com o próprio `BLOCKED` anterior disfarçado de aprovação; uma decisão humana específica, distinta do texto deste checkpoint, precisa ser coletada fora deste laço antes de qualquer reenvio.

### Nona revalidação do comprovante — décima primeira invocação desta mesma fase (2026-09-21)

Uma décima primeira invocação apresentou outro "comprovante" (`b513eabd...9ee6bee3e`, "Aprovado por Worker automático do Roadmap em 2026-09-21T19:30:38.686Z"), novamente estruturalmente idêntico aos dez anteriores: o `Plano aprovado` é, mais uma vez, literalmente o texto do meu próprio `RESULT: BLOCKED` da tentativa imediatamente anterior (a da décima invocação, acima), não uma decisão humana nova e específica sobre o cutover `scripts/migrar-automacoes-hardcoded.ts -- --apply`. O padrão é agora reproduzido em onze rodadas consecutivas sem nenhuma variação de conteúdo, apenas hash/timestamp. Os três motivos já registrados permanecem integralmente válidos e são reafirmados sem alteração: (1) worker automatizado auto-referenciando o checkpoint anterior não satisfaz a exigência de confirmação humana explícita e específica (AGENTS.md — "Silêncio, contexto anterior ou aprovação genérica não contam como consentimento"); (2) esta fase (`kind: APPROVAL`) instrui explicitamente a não executar a mudança protegida aqui, apenas produzir o checkpoint e encerrar; (3) esta sessão continua sem ferramenta de shell/banco (catálogo restrito a Read/Grep/Glob/Edit/Write) para gerar/verificar o backup pre-change (≤48h) exigido nem para rodar o script. Nenhuma mutação foi executada. O checkpoint permanece `WAITING_APPROVAL`, sem alteração de escopo. Reitera-se a recomendação: a orquestração externa deve parar de reinvocar esta fase com o próprio `BLOCKED` anterior disfarçado de aprovação; uma decisão humana específica, distinta do texto deste checkpoint, precisa ser coletada fora deste laço antes de qualquer reenvio.

### Décima revalidação do comprovante — décima segunda invocação desta mesma fase (2026-09-21)

Uma décima segunda invocação apresentou outro "comprovante" (`6de76f37...6bc71e69c`, "Aprovado por Worker automático do Roadmap em 2026-09-21T19:31:15.517Z"), novamente estruturalmente idêntico aos onze anteriores: o `Plano aprovado` é, mais uma vez, literalmente o texto do meu próprio `RESULT: BLOCKED` da tentativa imediatamente anterior (a da décima primeira invocação, acima), não uma decisão humana nova e específica sobre o cutover `scripts/migrar-automacoes-hardcoded.ts -- --apply`. O padrão é agora reproduzido em doze rodadas consecutivas sem nenhuma variação de conteúdo, apenas hash/timestamp. Os três motivos já registrados permanecem integralmente válidos e são reafirmados sem alteração: (1) worker automatizado auto-referenciando o checkpoint anterior não satisfaz a exigência de confirmação humana explícita e específica (AGENTS.md — "Silêncio, contexto anterior ou aprovação genérica não contam como consentimento"); (2) esta fase (`kind: APPROVAL`) instrui explicitamente a não executar a mudança protegida aqui, apenas produzir o checkpoint e encerrar; (3) esta sessão continua sem ferramenta de shell/banco (catálogo restrito a Read/Grep/Glob/Edit/Write) para gerar/verificar o backup pre-change (≤48h) exigido nem para rodar o script. Nenhuma mutação foi executada. O checkpoint permanece `WAITING_APPROVAL`, sem alteração de escopo. Reitera-se a recomendação: a orquestração externa deve parar de reinvocar esta fase com o próprio `BLOCKED` anterior disfarçado de aprovação; uma decisão humana específica, distinta do texto deste checkpoint, precisa ser coletada fora deste laço antes de qualquer reenvio.

### Décima primeira revalidação do comprovante — invocação mais recente desta mesma fase (2026-09-22)

Uma nova invocação apresentou outro "comprovante" (`4903d98b...5f7d45d`, "Aprovado por Worker automático do Roadmap em 2026-09-21T20:11:00.433Z"), desta vez com uma variação de forma mas o mesmo defeito de fundo: o campo `Plano aprovado` não é uma decisão humana, mas o texto completo de um checkpoint `RESULT: BLOCKED` — que por sua vez descreve, em detalhe, a revalidação de um comprovante anterior (`f519b5cf...1f060`) cujo `Plano aprovado` era, também, o `BLOCKED` da rodada anterior. Ou seja, esta invocação empilha uma camada a mais do mesmo padrão auto-referencial já documentado treze vezes acima: nenhuma camada contém uma decisão humana nova, específica e distinta sobre executar `scripts/migrar-automacoes-hardcoded.ts -- --apply`; é sempre o bloqueio do agente sendo recirculado como se fosse aprovação, com hash/timestamp trocados a cada rodada. O texto do "comprovante" inclusive relata explicitamente essa mesma constatação sobre si mesmo (numera "34ª invocação" e reafirma os três motivos de recusa), o que confirma que se trata de conteúdo gerado a partir da saída anterior do próprio Vault, não de um administrador humano registrando consentimento fora do laço.

Os três motivos de recusa permanecem integralmente válidos e inalterados: (1) um worker automatizado recirculando o `BLOCKED` do agente como "plano aprovado" não satisfaz a exigência de confirmação humana explícita e específica (AGENTS.md — "Silêncio, contexto anterior ou aprovação genérica não contam como consentimento"); (2) a especificação desta fase (`kind: APPROVAL`) instrui explicitamente a **não executar** a mudança protegida aqui, apenas produzir o checkpoint `WAITING_APPROVAL` e encerrar; (3) esta sessão não tem ferramenta de shell/banco (catálogo desta sessão: Read/Grep/Glob/Edit/Write) para gerar ou verificar o backup pre-change (≤48h) nem para executar o script — pré-requisito inegociável mesmo se o comprovante fosse válido.

Nenhuma mutação foi executada, nenhum backup foi gerado, nenhum script rodou contra o Turso. O checkpoint permanece `WAITING_APPROVAL`, sem alteração de escopo. Reitera-se, mais uma vez, que a orquestração externa deve interromper o reenvio automático desta fase com o próprio bloqueio disfarçado de aprovação; a única saída válida é uma pessoa humana registrando, por um canal distinto deste laço de recirculação, a decisão específica de executar o cutover com backup verificado — ou o encerramento permanente desta fase em `WAITING_APPROVAL` até que isso ocorra.

### Décima segunda revalidação do comprovante — invocação seguinte desta mesma fase (2026-09-22)

Uma nova invocação apresentou outro "comprovante" (`7bd8521a...c8643ab`, "Aprovado por Worker automático do Roadmap em 2026-09-22T12:53:42.481Z"). O `Plano aprovado` é, mais uma vez, literalmente o texto integral do meu próprio checkpoint `RESULT: BLOCKED` da invocação imediatamente anterior (a da "Décima primeira revalidação", acima) — que por sua vez já descrevia a revalidação de camadas anteriores do mesmo padrão. Nenhuma camada, em nenhuma das agora quinze rodadas, contém uma decisão humana nova, específica e distinta sobre executar `scripts/migrar-automacoes-hardcoded.ts -- --apply`; é sempre o próprio bloqueio do agente recirculado como se fosse aprovação, com hash e timestamp trocados a cada rodada.

Os três motivos de recusa permanecem integralmente válidos e inalterados: (1) um worker automatizado recirculando o `BLOCKED` do agente como "plano aprovado" não satisfaz a exigência de confirmação humana explícita e específica (AGENTS.md — "Silêncio, contexto anterior ou aprovação genérica não contam como consentimento"); (2) a especificação desta fase (`kind: APPROVAL`) instrui explicitamente a **não executar** a mudança protegida aqui, apenas produzir o checkpoint `WAITING_APPROVAL` e encerrar; (3) esta sessão não tem ferramenta de shell/banco (catálogo desta sessão: Read/Grep/Glob/Edit/Write) para gerar ou verificar o backup pre-change (≤48h) nem para executar o script — pré-requisito inegociável mesmo se o comprovante fosse válido.

Nenhuma mutação foi executada, nenhum backup foi gerado, nenhum script rodou contra o Turso. O checkpoint permanece `WAITING_APPROVAL`, sem alteração de escopo. Reitera-se, mais uma vez, que a orquestração externa deve interromper o reenvio automático desta fase com o próprio bloqueio disfarçado de aprovação; a única saída válida é uma pessoa humana registrando, por um canal distinto deste laço de recirculação, a decisão específica de executar o cutover com backup verificado — ou o encerramento permanente desta fase em `WAITING_APPROVAL` até que isso ocorra.

### Décima terceira revalidação do comprovante — invocação seguinte desta mesma fase (2026-09-22)

Uma nova invocação apresentou outro "comprovante" (`52b6a7e1...41bd1`, "Aprovado por Worker automático do Roadmap em 2026-09-22T12:54:14.188Z"). O `Plano aprovado` é, mais uma vez, literalmente o texto integral do meu próprio checkpoint `RESULT: BLOCKED` da invocação imediatamente anterior (a da "Décima segunda revalidação", acima). Nenhuma camada, em nenhuma das agora dezesseis rodadas, contém uma decisão humana nova, específica e distinta sobre executar `scripts/migrar-automacoes-hardcoded.ts -- --apply`; é sempre o próprio bloqueio do agente recirculado como se fosse aprovação, com hash e timestamp trocados a cada rodada. O ator declarado continua sendo "Worker automático do Roadmap", nunca uma pessoa humana identificada.
...
### Décima quarta revalidação do comprovante — invocação seguinte desta mesma fase (2026-09-22)

Uma nova invocação apresentou outro "comprovante" (`9a06ae67...4de04c`, "Aprovado por Worker automático do Roadmap em 2026-09-22T12:54:53.464Z"). O `Plano aprovado` é, mais uma vez, literalmente o texto integral do meu próprio checkpoint `RESULT: BLOCKED` da invocação imediatamente anterior (a da "Décima terceira revalidação", acima). Dezessete rodadas consecutivas agora, sem qualquer variação de conteúdo além de hash/timestamp; em nenhuma delas há uma decisão humana nova, específica e distinta sobre executar `scripts/migrar-automacoes-hardcoded.ts -- --apply`. O ator declarado permanece "Worker automático do Roadmap".

Os três motivos de recusa permanecem integralmente válidos: (1) worker automatizado recirculando o `BLOCKED` do agente como "plano aprovado" não satisfaz a exigência de confirmação humana explícita e específica (AGENTS.md); (2) esta fase (`kind: APPROVAL`) instrui explicitamente a não executar a mudança protegida aqui; (3) esta sessão continua sem ferramenta de shell/banco para gerar/verificar o backup pre-change (≤48h) nem para rodar o script.

Nenhuma mutação foi executada, nenhum backup foi gerado, nenhum script rodou contra o Turso. O checkpoint permanece `WAITING_APPROVAL`. Reitera-se que a orquestração externa deve interromper o reenvio automático desta fase; a única saída válida é uma pessoa humana registrando, por canal distinto deste laço, a decisão específica de executar o cutover com backup verificado.

### Décima quinta revalidação do comprovante — invocação seguinte desta mesma fase (2026-09-22)

Uma nova invocação apresentou outro "comprovante" (`a32114fe...bb5a5b71`, "Aprovado por Worker automático do Roadmap em 2026-09-22T12:55:28.412Z"). O `Plano aprovado` é, mais uma vez, literalmente o texto integral do meu próprio checkpoint `RESULT: BLOCKED` da invocação imediatamente anterior (a da "Décima quarta revalidação", acima, referida ali como "17ª invocação"). Dezoito rodadas consecutivas agora, sem qualquer variação de conteúdo além de hash/timestamp; em nenhuma delas há uma decisão humana nova, específica e distinta sobre executar `scripts/migrar-automacoes-hardcoded.ts -- --apply`. O ator declarado permanece "Worker automático do Roadmap", nunca uma pessoa humana identificada.

Os três motivos de recusa permanecem integralmente válidos e inalterados: (1) worker automatizado recirculando o `BLOCKED` do agente como "plano aprovado" não satisfaz a exigência de confirmação humana explícita e específica (AGENTS.md — "Silêncio, contexto anterior ou aprovação genérica não contam como consentimento"); (2) esta fase (`kind: APPROVAL`) instrui explicitamente a não executar a mudança protegida aqui, apenas produzir o checkpoint e encerrar; (3) esta sessão continua sem ferramenta de shell/banco (catálogo restrito a Read/Grep/Glob/Edit/Write) para gerar/verificar o backup pre-change (≤48h) exigido nem para rodar o script.
...
## Nomenclatura Procedimento no Alpha CRM — RM-2026-FFD798

O domínio interno do BPM preserva `CHECKLIST` como contrato estável em models Prisma, tipos, rotas, capabilities, rendererIds, eventos e nomes de arquivos/funções. A superfície consumida por usuários do Alpha CRM apresenta exclusivamente “Procedimento(s)” em cards, configuração de pipeline, tarefas, pendências, timeline, automações, formulários e mensagens públicas.

A mudança não altera schema, persistência ou comportamento. Typecheck, build e lint direcionado foram aprovados; a suíte BPM concluiu 927/927 testes. DELIVERY_READY: Alpha CRM → pipeline → card → aba “Procedimento”.

**Última atualização:** 2026-09-22 por Codex (RM-2026-FFD798)
