...
**Achado em `Checklist`/`operacional_clientes`, corrigido em 2026-08-21 (mesmo dia):** `operacional_clientes_old_fase35` (4 linhas) tinha só 1 ID em comum com `operacional_clientes` atual — 3 empresas nunca migraram: `ADICEL - INDUSTRIA E COMERCIO LTDA`, `ARCOS DOURADOS COMERCIO DE ALIMENTOS SA`, e `ALPHA COMEX BRASIL LTDA` de novo (mesma empresa já restaurada em Extratos nesta sessão). Confirmado pelo usuário que o módulo Alpha CheckList ainda não foi lançado oficialmente para uso (dado de teste) — autorização para corrigir sem cautela extra de produto, mantendo a cautela técnica normal (backup, preservar tudo). Resolvido criando `Cliente` novo para ADICEL (id 293) e Arcos Dourados (id 294); Alpha Comex reaproveitou `Cliente.id: 288`. As 3 linhas migradas para o formato novo de `operacional_clientes` (`clienteOperacionalId` = login antigo preservado, `clienteId` = empresa nova), `Checklist` recriada com FK correta, 3 `Checklist` reais preservados intactos. Validado via Prisma Client (listagem + create/delete de teste). Backup: `database-backups/pre-change/painelalpha_turso_pre_change_checklist-operacional-fk-fix_2026-08-21T19-49-03-778Z.sql`.

**RESULTADO FINAL DESTA INVESTIGAÇÃO: as 11 relações de FK física órfã encontradas foram todas corrigidas** (`PeriodosAnalise`, `observacoes_contratos`, `CommissionEvent.businessProcessId`, `CommissionEntry`, `CommissionDivergence`, 8 tabelas de `BpmCard`, `Checklist`). Varredura final confirma zero tabela ativa com FK presa em nome `_old`/`_fixfk`.
