...
|---|---|---|
| Metas/Comercial | `ContratoComercial` (mantém nome) | `valorContrato`, `formaPagamento`, `canalAquisicao`, `closerNome`, `status` (ENVIADO/FECHADO), `pagamentoConfirmado*`, `contratoAssinado`, `contaComVenda`, `mes`/`ano`, `usuarioId`. Ganha `clienteId Int → Cliente` **obrigatório** (decisão do usuário, 2026-08-08 — `criarContrato` resolve/cria `Cliente` já na criação do contrato, não mais só no fechamento; ver implicações de fluxo na pergunta 5). **Perde** `cnpj`/`razaoSocial`/`nomeFantasia`/`dataConstituicao`/`regimeTributario`/`uf` como campos próprios — todos passam a vir via `include: { cliente }`. |
...
4. ~~**`OperacionalClientes.clienteId` obrigatório ou opcional?**~~ **RESPONDIDO (2026-08-08): obrigatório (`clienteId Int`, `NOT NULL`).** Toda empresa cadastrada no módulo Operacional passa a exigir um `Cliente` master vinculado. **Consequência real de fluxo:** `vincularEmpresaAoCliente` (`src/actions/ClientesOperacional.ts`) — hoje faz `create` direto em `OperacionalClientes` com cnpj/razaoSocial/nomeFantasia próprios — precisa mudar para **resolver/criar `Cliente` primeiro** (buscar por CNPJ normalizado; se não existir, criar) e só então criar `OperacionalClientes` já com `clienteId` preenchido, na mesma transação. Isso também significa que **toda empresa que hoje só existe no Operacional (nunca passou pelo CS&NPS) vai gerar um `Cliente` novo no backfill da Fase 2** — a Fase 1 precisa medir esse volume (quantos CNPJs de `OperacionalClientes` não têm hoje correspondente em `clientes`) para o usuário ter noção de quantos `Cliente`s "nascem" só por causa do Operacional, antes do backfill rodar de fato.
5. ~~**`ContratoComercial.clienteId` obrigatório ou opcional?**~~ **RESPONDIDO (2026-08-08): obrigatório — cria `Cliente` no ato.** Muda o fluxo de negócio atual: hoje um contrato existe desde "Enviado" e só gera registro em `clientes` na confirmação de pagamento (`confirmarFechamento` → `criarRegistroClienteAPartirDeContrato`). Com a FK obrigatória, **`criarContrato` (`src/actions/ContratoComercial.ts`) passa a resolver/criar `Cliente` já na criação do contrato**, não mais só no fechamento. **Implicações a decidir na Fase 3.6 (não antecipadas aqui, registradas como ponto de atenção):** (a) contratos "Enviados" que nunca fecham/são cancelados também vão gerar um `Cliente` — isso é uma mudança de semântica real (`Cliente` deixa de significar "cliente que fechou/está em relação ativa" e passa a significar "qualquer empresa que já teve uma proposta comercial", mais próximo de um cadastro de prospecção/lead); (b) `criarRegistroClienteAPartirDeContrato` deixa de ser o único ponto de criação — passa a ser só o ponto que cria/atualiza o `ClienteServico` (o serviço em si), já que o `Cliente` (empresa) nasce antes, na criação do contrato; (c) o merge de dados fiscais que hoje é feito na confirmação (nunca sobrescrever campo preenchido com vazio) precisa ser reavaliado para o momento da criação do contrato, que tem menos dado disponível (a consulta à Receita Federal é feita no mesmo formulário, então na prática o dado já costuma estar presente — mas confirmar na Fase 1/3).
...
4. **Pré-Análise/Radar/Radar Fiscal** (`ConsultaPreAnalise`, `consultas_radar`→`ConsultaRadarHistorico`, `radar_fiscal`) — 3 tabelas relacionadas, maior volume de linhas (histórico de consultas), mas escrita concentrada em poucos arquivos (`RadarAction.ts`, `NovoRadar.ts`, `PreAnalise.ts`, `RadarFiscal.ts`). Mantêm snapshot próprio (decisão da pergunta 3), então ganham `clienteId` mas não perdem seus campos cadastrais.
...
6. **Metas/Comercial** (`ContratoComercial.clienteId` **obrigatório** — decisão do usuário, pergunta 5: `criarContrato` passa a resolver/criar `Cliente` no ato da criação do contrato, não mais só no fechamento; `criarRegistroClienteAPartirDeContrato` é reescrita para lidar só com `ClienteServico`). Maior superfície de Server Actions (11+ funções em `ContratoComercial.ts` + `Metas.ts` + adapters de Comissões que leem contratos).
