...
describe("GET /PainelAlpha/GeradorDocumentos/[id]/download", () => {
  ...
  it("retorna 401 quando não autenticado", async () => {
    ...
    const res = await GET(new Request("http://localhost/api"), { params: Promise.resolve({ id: "doc1" }) });
  ...
  it("retorna 404 quando o documento não existe", async () => {
    ...
    const res = await GET(new Request("http://localhost/api"), { params: Promise.resolve({ id: "inexistente" }) });
