...
describe("contratos do Roadmap Alpha", () => {
  it("valida edição material sem permitir prioridade no contrato de conteúdo", () => {
    const content = {
      ...
      description: "Uma edição material deve criar uma nova revisão da documentação.",
    ...
    expect(roadmapObjectiveContentSchema.parse(content).moduleKey).toBe("roadmap");
    expect(roadmapObjectiveContentSchema.safeParse({ ...content, globalPriority: 2 }).success).toBe(false);
  });
  ...
  it("rejeita módulo desconhecido e chaves extras", () => {
    expect(roadmapObjectiveInputSchema.safeParse({
      contractVersion: 1,
  ...
  it("aceita manifesto contínuo com contexto e execução", () => {
    expect(roadmapPhaseManifestSchema.parse(validManifest()).phases).toHaveLength(2);
  });
  ...
  it("gera filename internamente com numeração de dois dígitos", () => {
    expect(roadmapPhaseFilename({ number: 1, slug: "scout-descoberta" })).toBe("01-scout-descoberta.md");
  });
});
