...
describe("domínio de checklists BPM", () => {
  it("considera todas as dimensões nulas como compatíveis", () => {
    expect(templateChecklistCompativel({ pipelineId: null, etapaId: null, cardId: null }, card)).toBe(true);
  });

  it("exige correspondência somente de pipeline, etapa e card", () => {
    expect(templateChecklistCompativel({ pipelineId: "pipe-1", etapaId: "etapa-1", cardId: "card-1" }, card)).toBe(true);
    expect(templateChecklistCompativel({ pipelineId: "pipe-2", etapaId: null, cardId: null }, card)).toBe(false);
    const escopoLegado = { pipelineId: null, etapaId: null, cardId: null, servico: "Comercial", tipoProcesso: "Exportação" };
    expect(templateChecklistCompativel(escopoLegado, card)).toBe(true);
  });
