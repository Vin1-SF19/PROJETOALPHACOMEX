...
describe("workspace integral de configuração", () => {
  ...
  it("não confunde editores independentes com publicação parcial do rascunho principal", () => {
    ...
    const visibilidade = ler(
      "src/app/PainelAlpha/AlphaCRM/admin/pipelines/[pipelineId]/VisibilidadeEtapasSection.tsx",
    );
    expect(admin).not.toContain(
      "if (editandoCampoId) {\n      void salvarEdicao",
    );
    expect(admin).not.toContain("Publicar campo");
    ...
    expect(cadencia).toContain(
      "A seleção fica pendente até a publicação explícita",
    );
  ...
  it("invalida versões abertas quando outro escritor de configuração publica", () => {
    for (const arquivo of [
      ...
      expect(ler(`src/actions/bpm/${arquivo}.ts`)).toContain(
        "avancarConfigVersionBpm",
      );
    }
  });

  it("mantém o resolvedor do runtime sem expor a simulação administrativa", () => {
    ...
    expect(ui).not.toContain("Simulação real do runtime");
    expect(ui).not.toContain("SimularConfiguracaoSlaBpm");
  });
  ...
  it("não serializa valores brutos de auditoria para o browser", () => {
    ...
    const bloco = pipelineAction.slice(
      pipelineAction.indexOf("configAuditoria:"),
      pipelineAction.indexOf("configAuditoria:") + 350,
    );
