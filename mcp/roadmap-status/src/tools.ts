...
export const roadmapTools = [
  {
    ...
    description:
      "Lista as fases (RoadmapProductionRun) em prioridade global e ordem de fase. Se uma fase falhar ou bloquear, não pule para o objetivo seguinte.",
    inputSchema: {
      status: z
        .enum([
          "PENDING",
  ...
  },
  {
    ...
    description:
      "Retorna o detalhe completo de uma RoadmapProductionRun: objetivo, artefato documentado e últimos eventos.",
  ...
  },
  {
    ...
    description:
      "Transiciona a fase para IN_PROGRESS (a partir de PENDING). O servidor rejeita o início se uma fase anterior ou objetivo mais prioritário ainda não concluiu com sucesso.",
    inputSchema: { runId: z.string().min(1) },
  ...
  },
  {
    ...
    description:
      "Transiciona a fase para SUCCEEDED, registrando um resumo do que foi feito.",
    inputSchema: {
      runId: z.string().min(1),
      resultSummary: z.string().min(1).max(4000),
    },
    execute: (args: { runId: string; resultSummary: string }) =>
      roadmapApiClient.updateStatus(
        args.runId,
        "SUCCEEDED",
        args.resultSummary,
      ),
  },
  {
    ...
    description:
      "Transiciona a fase para FAILED, registrando código e resumo do erro.",
    inputSchema: {
      runId: z.string().min(1),
  ...
  {
    name: "roadmap_tentar_fase_novamente",
    title: "Tentar novamente a fase com falha",
    description:
      "Move uma fase FAILED de volta para PENDING. Retome esta mesma fase; não avance para outra enquanto ela não concluir com sucesso.",
    inputSchema: { runId: z.string().min(1) },
    execute: (args: { runId: string }) =>
      roadmapApiClient.updateStatus(args.runId, "PENDING"),
  },
  {
    name: "roadmap_perguntar",
    title: "Registrar pergunta e marcar fase como aguardando decisão",
    description:
      "Registra uma pergunta/pedido de decisão e transiciona a fase para NEEDS_INPUT.",
    inputSchema: {
      runId: z.string().min(1),
  ...
  },
  {
    ...
    description:
      "Adiciona uma nota livre ao histórico da fase, sem mudar o status.",
    inputSchema: {
      runId: z.string().min(1),
      content: z.string().min(1).max(4000),
    },
  ...
  },
  {
    ...
    description:
      "Lista o log append-only de eventos (mudanças de status, mensagens, notas) de uma fase.",
    inputSchema: {
      runId: z.string().min(1),
  ...
  },
  {
    ...
    execute: (args: { objectiveId: string; reportMarkdown: string }) =>
      roadmapApiClient.setCompletionReport(
        args.objectiveId,
        args.reportMarkdown,
      ),
  },
