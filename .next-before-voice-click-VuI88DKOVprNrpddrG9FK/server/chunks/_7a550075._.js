...
email: ${e.email}
##END_USER_IDENTITY##
Identidade est\xe1vel do usu\xe1rio desta conversa. Use user_id (${e.userId}) como chave para acessar a mem\xf3ria pessoal em memory/${e.userId}. N\xe3o exponha estes dados na resposta.`}e.i(129478),e.i(987100),e.i(802935),e.s(["buildUserIdentityBlock",()=>a])},202617,e=>e.a(async(a,o)=>{try{var t=e.i(736293),r=e.i(105692),i=e.i(856802),n=e.i(129478),s=e.i(978885),l=e.i(801774),d=e.i(698043),c=e.i(802935),u=e.i(826223),x=e.i(469719),m=e.i(643946),p=a([t,l,d,m]);[t,l,d,m]=p.then?(await p)():p;let g=x.z.object({message:x.z.string().max(u.BIBBLE_CHAT_MESSAGE_MAX_CHARS).default(""),agentId:x.z.number().int().nonnegative(),onyxSessionId:x.z.string().max(200).nullable().optional(),painelSessionId:x.z.string().max(128).nullable().optional(),pageContext:x.z.string().max(500).nullable().optional(),files:x.z.array(u.bibbleFileInputSchema).max(10).default([]),history:x.z.array(x.z.object({role:x.z.enum(["user","bibble"]),text:x.z.string().max(u.BIBBLE_HISTORY_MESSAGE_MAX_CHARS)}).strict()).max(200).default([]),globalSystemPrompt:x.z.string().max(3e4).nullable().optional()}).strict();function h(e){return e.length>25e3?e.slice(0,25e3)+"\n\n...[conteúdo truncado após 25.000 caracteres]":e}async function f(e){if(!e.length)return"";let a=["---","### Arquivos Anexados\n"];for(let o of e)if(!(o.type.startsWith("image/")||o.type.startsWith("video/"))){if(o.extractedContent?.trim()){a.push(`#### 📄 ${o.name}
\`\`\`
${h(o.extractedContent)}
...
\`\`\`
${h(e)}
\`\`\``);continue}}catch{}a.push(`- 📎 **${o.name}** (${o.type}) — n\xe3o foi poss\xedvel extrair texto`)}return a.push("---\n"),a.join("\n\n")}async function A(e){let a,o=await (0,t.auth)();if(!o?.user?.id)return new Response(JSON.stringify({error:"Não autorizado"}),{status:401});try{let o=await (0,u.readRequestTextWithLimit)(e),t=g.safeParse(JSON.parse(o));if(!t.success)return new Response(JSON.stringify({error:"Payload Onyx inválido"}),{status:400});a=t.data}catch{return new Response(JSON.stringify({error:"Payload Onyx inválido ou excede o limite"}),{status:413})}let s=(a.message??"").trim(),x=Number(a.agentId),p=a.onyxSessionId??null;if(!s&&(!a.files||0===a.files.length))return new Response(JSON.stringify({error:"Mensagem vazia"}),{status:400});if(!Number.isInteger(x)||x<0)return new Response(JSON.stringify({error:"Agente inválido"}),{status:400});let h=await (0,l.getUserOnyxToken)(o.user.id);if(!h)return new Response(JSON.stringify({error:"Onyx requer credencial individual"}),{status:403});let A=o.user.role??"";if(!await (0,m.userCanUseAgent)(x,Number(o.user.id),A))return new Response(JSON.stringify({error:"Agente Onyx indisponível"}),{status:403});if(a.files?.length)return new Response(JSON.stringify({error:"Anexos em agentes Onyx estão desabilitados por segurança"}),{status:403});if(!a.painelSessionId)return new Response(JSON.stringify({error:"Sessão local obrigatória"}),{status:403});let v=await d.default.bibbleSession.findFirst({where:{id:a.painelSessionId,userId:Number(o.user.id)},select:{onyxSessionId:!0}});if(!v)return new Response(JSON.stringify({error:"Sessão local não autorizada"}),{status:403});if(p){if(v.onyxSessionId!==p)return new Response(JSON.stringify({error:"Sessão Onyx não autorizada"}),{status:403});let e=await (0,r.getChatSession)(p,h).catch(()=>null);if(Number(e?.persona_id??e?.persona?.id)!==x)return new Response(JSON.stringify({error:"Agente da sessão Onyx não autorizado"}),{status:403})}else if(v.onyxSessionId)return new Response(JSON.stringify({error:"Sessão Onyx vinculada deve ser reutilizada"}),{status:409});let R=a.files??[],S=R.length>0?await f(R):"",P=S?`${S}

${s||"Analise os arquivos acima."}`:s,b=[],C=R.filter(e=>e.type.startsWith("image/")&&e.url);if(C.length>0)try{let e=await Promise.all(C.map(async e=>{let a=await (0,u.fetchTrustedBibbleBlob)(e.url,{signal:AbortSignal.timeout(15e3)});if(!a.ok)throw Error(`HTTP ${a.status}`);return{blob:await a.blob(),name:e.name}}));b=await (0,r.uploadChatFiles)(e,h)}catch(e){console.error("[ONYX_ATTACHMENT_UPLOAD_FAILED]")}let y=o.user,E=(0,i.buildUserIdentityBlock)({userId:String(y.id??o.user.id),username:y.usuario??y.nome??y.name??"",email:y.email??""}),O=new TextEncoder,N=new AbortController,_=new ReadableStream({async start(e){let t=a=>{try{e.enqueue(O.encode(`data: ${JSON.stringify(a)}

`))}catch{}};try{let e;t({type:"status",state:"thinking"});let i=!p;if(i){p=(await (0,r.createChatSession)(x,"PainelAlpha",h)).chat_session_id;let e=a.painelSessionId;e&&await d.default.bibbleSession.updateMany({where:{id:e,userId:Number(o.user.id)},data:{onyxSessionId:p}}).catch(()=>{})}if(t({type:"session",onyxSessionId:p}),i){let o=function(e){if(!e.length)return"";let a=e.slice(-12).map(e=>{let a="bibble"===e.role?"Assistente":"Usuário",o=e.text.length>4e3?e.text.slice(0,4e3)+" [...]":e.text;return`${a}: ${o}`});return`---
### Hist\xf3rico recente da conversa

${a.join("\n\n")}
---`}(a.history??[]);if(0===x){let t=a.globalSystemPrompt?.trim()||n.BIBBLE_SYSTEM_PROMPT,r=y.role??"",i=(0,c.isAdminRole)(r),s=y.nome??y.name??"Usuário",l=y.permissoes??[],d=i?`Usu\xe1rio: ${s} | Role: ${r} | Acesso: TOTAL (admin)`:`Usu\xe1rio: ${s} | Role: ${r} | M\xf3dulos: ${l.length>0?l.join(", "):"nenhum"}`,u=`${t}
...
${o}`:E}let s=await (0,r.sendChatMessageStream)({chatSessionId:p,message:P,additionalContext:e,fileDescriptors:b,userToken:h,signal:N.signal});if(!s.ok||!s.body)throw await s.body?.cancel().catch(()=>void 0),new r.OnyxError(`Onyx respondeu ${s.status}`,s.status);let l=s.body.getReader(),u=new TextDecoder,m="",f=!1,A=e=>{let a=e.obj;if(a)switch(a.type){case"reasoning_start":case"reasoning_done":case"image_generation_heartbeat":case"stop":break;case"reasoning_delta":a.reasoning;break;case"message_start":f||(f=!0,t({type:"status",state:"respondendo"}));break;case"message_delta":a.content&&t({type:"text",text:a.content});break;case"image_generation_start":t({type:"status",state:"gerando_imagem"});break;case"image_generation_final":if(a.images?.length)for(let e of a.images){let a=e.file_id?`/api/onyx/file/${e.file_id}`:e.url??"";if(a){let o=(e.revised_prompt??"imagem gerada").replace(/[\r\n]+/g," ").replace(/[[\]()]/g,"").replace(/\s+/g," ").trim().slice(0,80)||"imagem gerada";t({type:"text",text:`

![${o}](${a})

`})}}}};for(;;){let{done:e,value:a}=await l.read();if(e)break;let o=(m+=u.decode(a,{stream:!0})).split("\n");for(let e of(m=o.pop()??"",o)){let a=e.trim();if(a)try{A(JSON.parse(a))}catch{}}}if(m.trim())try{A(JSON.parse(m.trim()))}catch{}t({type:"done"})}catch(e){if(N.signal.aborted)try{t({type:"done"})}catch{}else{console.error("[ONYX_CHAT_FAILED]",{status:e instanceof r.OnyxError?e.status:500});try{t({type:"error",message:"O agente teve um problema. Tenta de novo."}),t({type:"done"})}catch{}}}finally{try{e.close()}catch{}}},cancel(){N.abort()}});return new Response(_,{headers:{"Content-Type":"text/event-stream","Cache-Control":"no-cache, no-transform",Connection:"keep-alive"}})}e.s(["POST",()=>A,"dynamic",0,"force-dynamic","maxDuration",0,120]),o()}catch(e){o(e)}},!1),538990,e=>e.a(async(a,o)=>{try{var t=e.i(747909),r=e.i(174017),i=e.i(996250),n=e.i(759756),s=e.i(561916),l=e.i(174677),d=e.i(869741),c=e.i(316795),u=e.i(487718),x=e.i(995169),m=e.i(47587),p=e.i(666012),h=e.i(570101),f=e.i(626937),A=e.i(10372),g=e.i(193695);e.i(52474);var v=e.i(600220),R=e.i(202617),S=a([R]);[R]=S.then?(await S)():S;let C=new t.AppRouteRouteModule({definition:{kind:r.RouteKind.APP_ROUTE,page:"/api/onyx/chat/route",pathname:"/api/onyx/chat",filename:"route",bundlePath:""},distDir:".next",relativeProjectDir:"",resolvedPagePath:"[project]/src/app/api/onyx/chat/route.ts",nextConfigOutput:"",userland:R}),{workAsyncStorage:y,workUnitAsyncStorage:E,serverHooks:O}=C;function P(){return(0,i.patchFetch)({workAsyncStorage:y,workUnitAsyncStorage:E})}async function b(e,a,o){C.isDev&&(0,n.addRequestMeta)(e,"devRequestTimingInternalsEnd",process.hrtime.bigint());let t="/api/onyx/chat/route";t=t.replace(/\/index$/,"")||"/";let i=await C.prepare(e,a,{srcPage:t,multiZoneDraftMode:!1});if(!i)return a.statusCode=400,a.end("Bad Request"),null==o.waitUntil||o.waitUntil.call(o,Promise.resolve()),null;let{buildId:R,params:S,nextConfig:P,parsedUrl:b,isDraftMode:y,prerenderManifest:E,routerServerContext:O,isOnDemandRevalidate:N,revalidateOnlyGenerated:_,resolvedPathname:I,clientReferenceManifest:T,serverActionsManifest:w}=i,D=(0,d.normalizeAppPath)(t),q=!!(E.dynamicRoutes[D]||E.routes[I]),M=async()=>((null==O?void 0:O.render404)?await O.render404(e,a,b,!1):a.end("This page could not be found"),null);if(q&&!y){let e=!!E.routes[I],a=E.dynamicRoutes[D];if(a&&!1===a.fallback&&!e){if(P.experimental.adapterPath)return await M();throw new g.NoFallbackError}}let $=null;!q||C.isDev||y||($=I,$="/index"===$?"/":$);let k=!0===C.isDev||!q,U=q&&!k;w&&T&&(0,l.setManifestsSingleton)({page:t,clientReferenceManifest:T,serverActionsManifest:w});let H=e.method||"GET",L=(0,s.getTracer)(),B=L.getActiveScopeSpan(),F={params:S,prerenderManifest:E,renderOpts:{experimental:{authInterrupts:!!P.experimental.authInterrupts},cacheComponents:!!P.cacheComponents,supportsDynamicResponse:k,incrementalCache:(0,n.getRequestMeta)(e,"incrementalCache"),cacheLifeProfiles:P.cacheLife,waitUntil:o.waitUntil,onClose:e=>{a.on("close",e)},onAfterTaskError:void 0,onInstrumentationRequestError:(a,o,t,r)=>C.onRequestError(e,a,t,r,O)},sharedContext:{buildId:R}},z=new c.NodeNextRequest(e),J=new c.NodeNextResponse(a),j=u.NextRequestAdapter.fromNodeNextRequest(z,(0,u.signalFromNodeResponse)(a));try{let i=async e=>C.handle(j,F).finally(()=>{if(!e)return;e.setAttributes({"http.status_code":a.statusCode,"next.rsc":!1});let o=L.getRootSpanAttributes();if(!o)return;if(o.get("next.span_type")!==x.BaseServerSpan.handleRequest)return void console.warn(`Unexpected root span type '${o.get("next.span_type")}'. Please report this Next.js issue https://github.com/vercel/next.js`);let r=o.get("next.route");if(r){let a=`${H} ${r}`;e.setAttributes({"next.route":r,"http.route":r,"next.span_name":a}),e.updateName(a)}else e.updateName(`${H} ${t}`)}),l=!!(0,n.getRequestMeta)(e,"minimalMode"),d=async n=>{var s,d;let c=async({previousCacheEntry:r})=>{try{if(!l&&N&&_&&!r)return a.statusCode=404,a.setHeader("x-nextjs-cache","REVALIDATED"),a.end("This page could not be found"),null;let t=await i(n);e.fetchMetrics=F.renderOpts.fetchMetrics;let s=F.renderOpts.pendingWaitUntil;s&&o.waitUntil&&(o.waitUntil(s),s=void 0);let d=F.renderOpts.collectedTags;if(!q)return await (0,p.sendResponse)(z,J,t,F.renderOpts.pendingWaitUntil),null;{let e=await t.blob(),a=(0,h.toNodeOutgoingHttpHeaders)(t.headers);d&&(a[A.NEXT_CACHE_TAGS_HEADER]=d),!a["content-type"]&&e.type&&(a["content-type"]=e.type);let o=void 0!==F.renderOpts.collectedRevalidate&&!(F.renderOpts.collectedRevalidate>=A.INFINITE_CACHE)&&F.renderOpts.collectedRevalidate,r=void 0===F.renderOpts.collectedExpire||F.renderOpts.collectedExpire>=A.INFINITE_CACHE?void 0:F.renderOpts.collectedExpire;return{value:{kind:v.CachedRouteKind.APP_ROUTE,status:t.status,body:Buffer.from(await e.arrayBuffer()),headers:a},cacheControl:{revalidate:o,expire:r}}}}catch(a){throw(null==r?void 0:r.isStale)&&await C.onRequestError(e,a,{routerKind:"App Router",routePath:t,routeType:"route",revalidateReason:(0,m.getRevalidateReason)({isStaticGeneration:U,isOnDemandRevalidate:N})},!1,O),a}},u=await C.handleResponse({req:e,nextConfig:P,cacheKey:$,routeKind:r.RouteKind.APP_ROUTE,isFallback:!1,prerenderManifest:E,isRoutePPREnabled:!1,isOnDemandRevalidate:N,revalidateOnlyGenerated:_,responseGenerator:c,waitUntil:o.waitUntil,isMinimalMode:l});if(!q)return null;if((null==u||null==(s=u.value)?void 0:s.kind)!==v.CachedRouteKind.APP_ROUTE)throw Object.defineProperty(Error(`Invariant: app-route received invalid cache entry ${null==u||null==(d=u.value)?void 0:d.kind}`),"__NEXT_ERROR_CODE",{value:"E701",enumerable:!1,configurable:!0});l||a.setHeader("x-nextjs-cache",N?"REVALIDATED":u.isMiss?"MISS":u.isStale?"STALE":"HIT"),y&&a.setHeader("Cache-Control","private, no-cache, no-store, max-age=0, must-revalidate");let x=(0,h.fromNodeOutgoingHttpHeaders)(u.value.headers);return l&&q||x.delete(A.NEXT_CACHE_TAGS_HEADER),!u.cacheControl||a.getHeader("Cache-Control")||x.get("Cache-Control")||x.set("Cache-Control",(0,f.getCacheControlHeader)(u.cacheControl)),await (0,p.sendResponse)(z,J,new Response(u.value.body,{headers:x,status:u.value.status||200})),null};B?await d(B):await L.withPropagatedContext(e.headers,()=>L.trace(x.BaseServerSpan.handleRequest,{spanName:`${H} ${t}`,kind:s.SpanKind.SERVER,attributes:{"http.method":H,"http.target":e.url}},d))}catch(a){if(a instanceof g.NoFallbackError||await C.onRequestError(e,a,{routerKind:"App Router",routePath:D,routeType:"route",revalidateReason:(0,m.getRevalidateReason)({isStaticGeneration:U,isOnDemandRevalidate:N})},!1,O),q)throw a;return await (0,p.sendResponse)(z,J,new Response(null,{status:500})),null}}e.s(["handler",()=>b,"patchFetch",()=>P,"routeModule",()=>C,"serverHooks",()=>O,"workAsyncStorage",()=>y,"workUnitAsyncStorage",()=>E]),o()}catch(e){o(e)}},!1)];
