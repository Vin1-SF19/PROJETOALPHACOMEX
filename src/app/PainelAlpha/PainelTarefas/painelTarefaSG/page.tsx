...
type Tarefa = Awaited<ReturnType<typeof BuscarTarefasPorEmail>>[number];
...
export default function PainelTarefas() {
    ...
    async function carregarTudo() {
        ...
        const dados = await buscarListaCompra();
        setListaCompraDoBanco(dados);
    ...
    return (
        <div className="max-w-2xl mx-auto p-4 md:p-8 space-y-10 min-h-screen pb-20">
            {abaAtual === "TAREFAS" ? (
                <>
                    <header className="space-y-6 mb-10">
                        ...
                        <div className="flex items-center justify-between gap-4">
                            ...
                            <button
                                onClick={() => setAbaAtual((prev) => (prev === "TAREFAS" ? "COMPRAS" : "TAREFAS"))}
                                className={`flex items-center gap-3 px-6 py-3 rounded-[1.5rem] border transition-all duration-300 group ${(abaAtual as string) === "COMPRAS"
                            ...
                            >
                                <div className="relative">
                                    ...
                                    {produtos.filter(p => Number(p.quantidade) <= Number(p.estoqueMinimo)).length > 0 && abaAtual === "TAREFAS" && (
                                        <span className="absolute -top-2 -right-2 w-4 h-4 bg-rose-500 rounded-full flex items-center justify-center text-[8px] font-black text-white animate-pulse">
                                            {produtos.filter(p => Number(p.quantidade) <= Number(p.estoqueMinimo)).length}
                                        </span>
                                    )}
                                </div>
                ...
                <ListaCompras
                    itens={listaCompraDoBanco}
                    aoVoltar={() => setAbaAtual("TAREFAS")}
                    atualizarDados={carregarTudo}
                />
            )}
        </div>
    );
