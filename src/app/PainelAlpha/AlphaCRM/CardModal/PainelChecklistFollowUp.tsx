...
export function PainelChecklistFollowUp({ cardId, accent, onAtualizado, onEstadoChange, podeEditar }: PainelChecklistFollowUpProps) {
  ...
  }, [cardId, onEstadoChange, tentativa]);
        ...
        <div className="space-y-3 border-t border-white/5 pt-3">
          {checklist.perguntas.map((pergunta) => (
            <div key={pergunta.id} className="space-y-1.5">
              ...
              {pergunta.tipo === "selecao" ? (
                <select id={`follow-up-${pergunta.id}`} disabled={!podeEditar} className={inputClassName} value={valorTexto(respostas[pergunta.id])} onChange={(evento) => setRespostas((atuais) => ({ ...atuais, [pergunta.id]: evento.target.value }))}>
                  <option value="">Selecione...</option>
                  {pergunta.opcoes.map((opcao) => <option key={opcao} value={opcao}>{opcao}</option>)}
                </select>
              ) : pergunta.tipo === "booleano" ? (
                <select id={`follow-up-${pergunta.id}`} disabled={!podeEditar} className={inputClassName} value={typeof respostas[pergunta.id] === "boolean" ? String(respostas[pergunta.id]) : ""} onChange={(evento) => setRespostas((atuais) => {
                ...
                <textarea id={`follow-up-${pergunta.id}`} disabled={!podeEditar} className={`${inputClassName} min-h-24 resize-y`} value={valorTexto(respostas[pergunta.id])} onChange={(evento) => setRespostas((atuais) => ({ ...atuais, [pergunta.id]: evento.target.value }))} />
              )}
            </div>
          ))}
          ...
          {checklist.completo ? (
            ...
            <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
              <button type="button" onClick={() => void persistir(false)} disabled={!podeEditar || salvando} className="rounded-xl border border-white/10 px-3 py-2.5 text-xs font-semibold text-slate-300 disabled:opacity-50">Salvar rascunho</button>
