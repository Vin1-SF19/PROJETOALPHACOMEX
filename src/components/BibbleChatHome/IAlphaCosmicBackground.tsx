...
interface IAlphaCosmicBackgroundProps {
  accent: string;
  currentHour: number;
}
...
// meanLongitudeRate em graus por século juliano.
...
// Textura procedural: noise fractal SVG inline (feTurbulence), tileável e sem
// requests externos. baseFrequency anisotrópica ("x y") gera faixas de gás.
...
interface PlanetVisual {
  ...
  /** Gradientes extras sobre a superfície: continentes, calotas, crateras. */
  surfaceExtras?: string[];
  /** Superfície de gigante gasoso: faixas atmosféricas (ângulo já inclui o tilt). */
  bands?: string;
  /** Tempestade (Grande Mancha de Júpiter, mancha escura de Netuno). */
  spot?: string;
  clouds: boolean;
  /** Cor RGB do espalhamento atmosférico no limbo iluminado. */
  atmosphere: string;
  ...
  axialTilt: number;
  /** Período de rotação sideral real em horas; negativo = rotação retrógrada. */
...
const PLANET_VISUALS: Record<PlanetId, PlanetVisual> = {
  ...
  venus: {
    ...
    clouds: false,
    atmosphere: "244,220,170",
    atmosphereStrength: 0.55,
    noiseFrequency: "0.07 0.16",
    noiseOctaves: 4,
    noiseSeed: 23,
    textureOpacity: 0.5,
    textureBlend: "soft-light",
    axialTilt: -3,
    // Rotação retrógrada real de Vênus (~243 dias).
  ...
  jupiter: {
    ...
    bands:
      "linear-gradient(98deg, #c9a06b 0%, #eedec0 9%, #b98a52 17%, #e7d6b4 26%, #a5713f 35%, #f0e2c4 45%, #bd9059 54%, #e4d0ab 63%, #a97a45 72%, #ead9b8 82%, #b9895a 91%, #d9c39a 100%)",
    spot:
      "radial-gradient(ellipse at 63% 66%, rgba(196,90,58,0.8) 0%, rgba(196,90,58,0.35) 5%, transparent 11%)",
    clouds: false,
    atmosphere: "236,210,160",
    atmosphereStrength: 0.45,
    noiseFrequency: "0.02 0.22",
    noiseOctaves: 4,
    noiseSeed: 59,
    textureOpacity: 0.5,
    textureBlend: "soft-light",
    axialTilt: 8,
    // Júpiter é o rotador mais rápido do sistema (~9,9h).
  ...
  saturn: {
    ...
    bands:
      "linear-gradient(117deg, #d6b378 0%, #efe2c0 14%, #c8a263 28%, #ecdcb8 42%, #bb9355 57%, #ead9b5 72%, #cda868 86%, #e6d4ae 100%)",
    clouds: false,
    atmosphere: "240,220,170",
    atmosphereStrength: 0.4,
    noiseFrequency: "0.02 0.18",
    noiseOctaves: 3,
    noiseSeed: 71,
    textureOpacity: 0.42,
    textureBlend: "soft-light",
  ...
  uranus: {
    ...
    surfaceExtras: [
      // Faixas quase verticais: o eixo de Urano é deitado (~98°).
      "linear-gradient(172deg, rgba(190,238,246,0.35) 0%, rgba(110,195,212,0.12) 50%, rgba(190,238,246,0.35) 100%)",
    ],
    clouds: false,
    atmosphere: "150,230,245",
    atmosphereStrength: 0.5,
    noiseFrequency: "0.05 0.1",
    noiseOctaves: 3,
    noiseSeed: 83,
    textureOpacity: 0.3,
    textureBlend: "soft-light",
    axialTilt: 82,
    // Urano gira "de lado" e em sentido retrógrado.
...
// Composição da cena: sol deslocado do centro, plano orbital visto em perspectiva.
...
// Kepler: M = E - e·sin(E), resolvido por ponto fixo (converge bem para e <= 0.21).
...
// Distâncias reais (0.39 a 30 UA) comprimidas em escala log para caber na tela.
...
function computeCosmicScene(nowMs: number): CosmicScene {
  ...
  const planets = ORBITAL_ELEMENTS.map(elements => {
    ...
    // depth > 0 = metade da frente da elipse (embaixo na tela): maior, mais nítido.
...
// Temperaturas de cor: maioria branca/azulada, pouquíssimas quentes.
const STAR_TINTS = {
  white: "255,255,255",
  blueWhite: "219,234,254",
  blue: "165,200,252",
  warm: "253,224,168",
};
...
function toBoxShadow(stars: StarPoint[], spread: number, blurPx = 0) {
  return stars
    .map(star => `${star.horizontal}vw ${star.vertical}vh ${blurPx}px ${spread}px rgba(${star.tint},${star.opacity})`)
    .join(",");
}
...
const STAR_LAYERS = [
  {
    ...
    boxShadow: toBoxShadow(createStarLayer(173, 84), 0),
    duration: 8,
    driftScale: 1,
  },
  {
    id: "mid",
    className: "h-px w-px",
    boxShadow: toBoxShadow(createStarLayer(421, 126), 0),
    duration: 13,
    driftScale: 0.72,
  },
  {
    id: "far",
    className: "h-px w-px opacity-60",
    boxShadow: toBoxShadow(createStarLayer(911, 168), 0),
    duration: 19,
    driftScale: 0.44,
  },
  {
    id: "bright",
    className: "h-0.5 w-0.5",
    boxShadow: toBoxShadow(createStarLayer(557, 18), 0.5, 2),
    duration: 11,
    driftScale: 0.85,
  },
];
...
function computeSpin(rotationPeriodHours: number, epochMs: number): SpinState {
  const periodMs = Math.abs(rotationPeriodHours) * 3_600_000;
  const periodSeconds = periodMs / 1000 / SPIN_TIME_LAPSE;
  const phase = ((epochMs * SPIN_TIME_LAPSE) % periodMs) / periodMs;
  return {
    periodSeconds,
...
function spinAnimationStyle(spin: SpinState, reduceMotion: boolean): CSSProperties {
  if (reduceMotion) {
    return { backgroundPositionX: `${spin.offsetPx.toFixed(1)}px` };
  }
  return {
    ...
    // Delay negativo posiciona a animação exatamente na fase do relógio atual.
    animationDelay: `-${spin.delaySeconds.toFixed(1)}s`,
  };
}

export function IAlphaCosmicBackground({ accent, currentHour }: IAlphaCosmicBackgroundProps) {
  const reduceMotion = useReducedMotion();
  const hour = normalizeHour(currentHour);
  ...
  return (
    <div aria-hidden className="pointer-events-none absolute inset-0 z-0 overflow-hidden bg-[#020617]">
      ...
      {/* Faixa galáctica diagonal */}
      <div
        className="absolute left-[-20%] top-[6%] h-[58%] w-[140%] rotate-[-16deg] blur-3xl"
        style={{
          background:
            "linear-gradient(90deg, transparent 0%, rgba(96,165,250,0.07) 26%, rgba(147,197,253,0.13) 50%, rgba(96,165,250,0.07) 74%, transparent 100%)",
        }}
      />
      ...
      {/* Estrelas em camadas de paralaxe, com variação de cor e brilho */}
      {STAR_LAYERS.map(layer => (
        <motion.div
          key={layer.id}
          className={cn("absolute left-0 top-0 rounded-full", layer.className)}
      ...
      {/* Sol fora do centro, com halo quente contido e granulação sutil */}
      <div
        className="absolute -translate-x-1/2 -translate-y-1/2"
        style={{ left: `${SUN_POSITION.left}%`, top: `${SUN_POSITION.top}%`, zIndex: 10 }}
      >
        <div
        ...
        <motion.div
          ...
          style={{
            background:
              "radial-gradient(circle at 44% 40%, #fff7d6 0%, #fbbf24 30%, rgba(249,115,22,0.85) 58%, rgba(194,65,12,0.28) 78%, transparent 100%)",
      ...
      {/* Planetas nas longitudes heliocêntricas reais do momento atual */}
      {scene?.planets.map(planet => {
        const visual = PLANET_VISUALS[planet.id];
        const size = visual.size * planet.scale;

        // Iluminação direcional coerente: o lado claro aponta para o sol da cena.
        ...
        const baseSurface = visual.bands
          ? visual.bands
          : `radial-gradient(circle at ${lightX}% ${lightY}%, ${visual.palette.light} 0%, ${visual.palette.mid} 44%, ${visual.palette.dark} 80%)`;
        const surfaceBackground = [...(visual.surfaceExtras ?? []), baseSurface].join(",");

        // Rim atmosférico no limbo iluminado + brilho externo discreto.
        ...
        // Profundidade: planetas distantes perdem saturação, contraste e nitidez.
        ...
        const sphereFilter = [
          planet.blur > 0 ? `blur(${planet.blur}px)` : "",
          `saturate(${saturation})`,
          `contrast(${contrast})`,
        ].filter(Boolean).join(" ");
          ...
          >
            <motion.div
              ...
              animate={reduceMotion
                ...
                : { opacity: planet.opacity, y: [0, -visual.floatDistance, 0] }}
              transition={{
                opacity: { duration: 1.6, ease: "easeOut" },
                y: { duration: visual.floatDuration, repeat: Infinity, ease: "easeInOut" },
              }}
            >
              {/* Superfície: isolate limita os blend modes ao próprio planeta */}
              <div
                className="absolute inset-0 isolate overflow-hidden rounded-full"
                style={{ background: surfaceBackground, boxShadow: rimShadow }}
              >
                {/* Textura procedural: rotação própria com período sideral real
                    (acelerado) e fase ancorada na data/hora atual */}
                <div
                  data-spin={planet.id}
                ...
                {/* Nuvens da Terra: camada própria, velocidade diferente da superfície */}
                {cloudSpin && (
                  <div
                ...
                {/* Luz especular difusa vinda do sol */}
                <div
                  className="absolute inset-0 mix-blend-screen"
                  style={{
                    background: `radial-gradient(circle at ${lightX}% ${lightY}%, rgba(255,244,224,0.42) 0%, rgba(255,244,224,0.1) 34%, transparent 58%)`,
                  }}
                />

                {/* Terminador: lado oposto ao sol mergulha em sombra */}
                <div
                  className="absolute inset-0"
                  style={{
                    background: `radial-gradient(circle at ${shadeX}% ${shadeY}%, rgba(2,8,23,0.92) 0%, rgba(2,8,23,0.55) 32%, transparent 62%)`,
                  }}
                />
              </div>

              {/* Anéis de Saturno: faixas translúcidas com divisão de Cassini */}
              {visual.ring && (
                <div
                  ...
                  style={{
                    ...
                    background:
                      "radial-gradient(ellipse closest-side, transparent 55%, rgba(242,224,186,0.08) 58%, rgba(242,224,186,0.34) 66%, rgba(150,120,80,0.1) 72%, rgba(242,224,186,0.28) 79%, rgba(242,224,186,0.08) 86%, transparent 92%)",
                  }}
                />
              )}
      ...
      <motion.div
        ...
        transition={{ duration: 5, repeat: Infinity, repeatDelay: isNight ? 7 : 13, ease: "easeInOut" }}
      />

      {/* Vinheta para legibilidade do chat */}
