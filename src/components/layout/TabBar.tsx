...
export type Tab = { id: string; url: string; label: string; };
...
export default function TabBar({ tabs, activeId, onActivate, onClose }: TabBarProps) {
  ...
  return (
    <div className="h-10 bg-[#030813] border-b border-white/[0.06] flex items-center gap-0.5 px-2 overflow-x-auto shrink-0 [&::-webkit-scrollbar]:hidden" style={{ scrollbarWidth: 'none' }}>
      {tabs.map(tab => {
        ...
        return (
          <button
            ...
            className={`
              flex items-center gap-2 px-3 h-7 rounded-lg border transition-all duration-150 shrink-0 group/tab relative
              ${isActive
