...
import {
  AlternarTemplateChecklistBpm, CriarTemplateChecklistBpm, ExcluirTemplateChecklistBpm,
  ListarWorkspaceChecklistsBpm, SalvarTemplateChecklistBpm,
...
export function ChecklistsWorkspace({ workspace, erro, accent }: { workspace: Workspace; erro: string | null; accent: string }) {
  ...
  }, [busca, filtro, idsExcluidos, workspace.templates]);
  ...
  function confirmarExclusao() {
    ...
    startExclusaoTransition(async () => {
      const resposta = await ExcluirTemplateChecklistBpm({ id: excluindo.id, updatedAt: excluindo.updatedAt });
      if (!resposta.success) {
        const msg = resposta.error === "CONFLITO_CHECKLIST_TEMPLATE"
          ? "O procedimento foi alterado por outro usuário. Recarregue e tente novamente."
          : resposta.error;
        toast.error(msg);
        return;
      }
  ...
  return <main className="mx-auto w-full max-w-6xl space-y-5 p-4 sm:p-6">
    ...
    <Dialog open={Boolean(excluindo)} onOpenChange={(aberto) => { if (!aberto && !excluindoPendente) setExcluindo(null); }}>
      <DialogContent
        className="max-w-md"
        aria-busy={excluindoPendente}
      ...
      >
        <DialogHeader>
          ...
          <DialogDescription>
            Esta ação inativa o procedimento <strong className="text-slate-100">{excluindo?.nome}</strong>.
            Ele não será mais aplicado a novos cards. Procedimentos já materializados em cards existentes não serão afetados.
            {excluindo && excluindo._count.instancias > 0 && (
              <p className="mt-2 font-medium text-amber-300">
                Este procedimento está em uso por {excluindo._count.instancias} cards. Excluir mesmo assim?
              </p>
            )}
          </DialogDescription>
        </DialogHeader>
        <DialogFooter>
          <Button variant="ghost" onClick={() => setExcluindo(null)} disabled={excluindoPendente}>Cancelar</Button>
          <Button variant="destructive" onClick={confirmarExclusao} disabled={excluindoPendente}>{excluindoPendente ? "Excluindo…" : "Excluir procedimento"}</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  </main>;
