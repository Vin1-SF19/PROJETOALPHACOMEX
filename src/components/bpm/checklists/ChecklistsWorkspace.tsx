...
type Draft = { nome: string; descricao: string; ativo: boolean; pipelineId: string; etapaId: string; cardId: string; itens: DraftItem[] };
const VAZIO: Draft = { nome: "", descricao: "", ativo: true, pipelineId: "", etapaId: "", cardId: "", itens: [] };
...
export function ChecklistsWorkspace({ workspace, erro, accent }: { workspace: Workspace; erro: string | null; accent: string }) {
  ...
  const cards = useMemo(() => workspace.cards.filter((card) => (!draft.pipelineId || card.pipelineId === draft.pipelineId) && (!draft.etapaId || card.etapaId === draft.etapaId)), [draft.etapaId, draft.pipelineId, workspace.cards]);
  ...
  function salvar() {
    ...
    startTransition(async () => {
      const escopo = { nome: draft.nome.trim(), descricao: draft.descricao.trim() || null, ativo: draft.ativo, pipelineId: draft.pipelineId || null, etapaId: draft.etapaId || null, cardId: draft.cardId || null };
      if (editor === "NOVO") {
